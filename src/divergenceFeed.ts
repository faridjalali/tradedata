...
const miniChartDataCache = new Map<
  string,
  Array<{ time: string | number; open: number; high: number; low: number; close: number }>
>();
...
function refreshDivergenceCardsWhileRunning(force = false, timeframe?: '1d' | '1w'): void {
  ...
  divergenceTableLastUiRefreshAtMs = nowMs;
  if (timeframe) {
    void fetchDivergenceSignalsByTimeframe(timeframe)
      .then(() => renderDivergenceContainer(timeframe))
      .catch(() => {});
  } else {
    void fetchDivergenceSignals()
      .then(renderDivergenceOverview)
      .catch(() => {});
  }
}
