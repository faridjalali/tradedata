...
(async () => {
  ...
  for (const d of allDaily) {
    const dir = d.delta >= 0 ? '+' : '';
    console.log(`    ${d.date}: $${d.close.toFixed(2).padStart(6)}  ∂=${dir}${(d.delta/1000).toFixed(0).padStart(6)}K  (${d.deltaPct.toFixed(1).padStart(6)}%)  range=${d.rangePct.toFixed(2).padStart(6)}%  vol=${(d.totalVol/1e6).toFixed(2)}M`);
  }
  ...
  console.log(`  Price: $${accumCloses[0]?.toFixed(2)} → $${accumCloses[accumCloses.length-1]?.toFixed(2)} (${((accumCloses[accumCloses.length-1] - accumCloses[0]) / accumCloses[0] * 100).toFixed(1)}%)`);
  ...
  if (directResult) {
    ...
    if (directResult.deltaSlopeNorm !== undefined) console.log(`    Slope: ${directResult.deltaSlopeNorm.toFixed(2)}  Net∂: ${directResult.netDeltaPct.toFixed(2)}%  Corr: ${directResult.priceDeltaCorr?.toFixed(2)}  AccWk: ${directResult.accumWeeks}/${directResult.weeks}`);
    ...
    if (directResult.weeksData) {
      ...
      for (const w of directResult.weeksData) {
        ...
        console.log(`      ${w.weekStart}: ${dir}${w.deltaPct.toFixed(2)}% (${dir}${(w.delta/1000).toFixed(0)}K) [${w.nDays}d]`);
      }
    }
  } else {
  ...
  if (allResults.length > 0) {
    ...
    console.log(`    s5 Buy vs Sell (10%):     ${c.s5.toFixed(4)}  → contributes ${(c.s5 * 0.10).toFixed(4)}`);
    ...
    for (const w of best.weeksData) {
      ...
      console.log(`    ${w.weekStart}: ${dir}${(w.effectiveDelta/1000).toFixed(0)}K  ∂%=${dir}${w.deltaPct.toFixed(2)}%  [${w.nDays}d]${diff}`);
    }
    ...
    for (let i = 0; i < bestSlice.length; i++) {
      ...
      console.log(`    ${d.date}: $${d.close.toFixed(2).padStart(6)} (${priceChg.padStart(6)}%)  ∂=${dir}${(d.delta/1000).toFixed(0).padStart(6)}K  (${d.deltaPct.toFixed(1).padStart(6)}%)  rng=${d.rangePct.toFixed(2).padStart(6)}%  vol=${(d.totalVol/1e6).toFixed(2)}M${absorb}`);
    }
  } else {
    console.log(`\n${'='.repeat(80)}`);
    ...
    for (const w of allNon.slice(0, 15)) {
      ...
      console.log(`    ${w.startDate}→${w.endDate} (${w.winSize}d)  score=${w.score.toFixed(4)}  net∂=${w.netDeltaPct?.toFixed(2) || '?'}%  price=${w.overallPriceChange.toFixed(1)}%  slope=${w.deltaSlopeNorm?.toFixed(2) || '?'}${gated}`);
    }
  }
  ...
  for (const d of postBreakout) {
    const dir = d.delta >= 0 ? '+' : '';
    console.log(`    ${d.date}: $${d.close.toFixed(2).padStart(6)}  ∂=${dir}${(d.delta/1000).toFixed(0).padStart(6)}K  (${d.deltaPct.toFixed(1).padStart(6)}%)  vol=${(d.totalVol/1e6).toFixed(2)}M`);
  }
