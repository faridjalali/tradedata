...
function analyzeDistributionPhases(allDaily, breakoutDate) {
  ...
  for (let i = 10; i < preBreakout.length; i++) {
    ...
    if (priceChange > 3 && netDeltaPct < -5) {
      console.log(`    ${window[0].date}→${window[9].date}: price ${priceChange > 0 ? '+' : ''}${priceChange.toFixed(1)}%, delta ${netDeltaPct.toFixed(1)}% — DISTRIBUTION`);
    }
  }
  ...
  for (let i = 10; i < preBreakout.length; i++) {
    ...
    const netDeltaPct = totalVol > 0 ? (netDelta / totalVol) * 100 : 0;
    if (priceChange < -3 && netDeltaPct > 5) {
      console.log(`    ${window[0].date}→${window[9].date}: price ${priceChange.toFixed(1)}%, delta +${netDeltaPct.toFixed(1)}% — ACCUMULATION IN DECLINE`);
    }
  }
}
...
(async () => {
  ...
  if (breakoutIdx > 0 && breakoutIdx < allDaily.length - 10) {
    ...
    if (postZones.length > 0) {
      for (let zi = 0; zi < postZones.length; zi++) {
        ...
        console.log(`    Net∂: ${z.netDeltaPct.toFixed(2)}%  Price: ${z.overallPriceChange.toFixed(1)}%  Absorption: ${z.absorptionPct.toFixed(1)}%`);
      }
    } else {
  ...
  if (breakoutIdx > 0 && breakoutIdx < allDaily.length) {
    ...
    for (const w of postWeeks) {
      const dir = w.delta >= 0 ? '+' : '';
      const priceDir = w.close >= w.open ? '▲' : '▼';
      console.log(`    ${w.weekStart}: ${priceDir} $${w.open.toFixed(2)}→$${w.close.toFixed(2)}  ∂=${dir}${w.deltaPct.toFixed(2).padStart(7)}%  (${dir}${(w.delta/1000).toFixed(0).padStart(7)}K)  vol=${(w.avgVol/1e6).toFixed(2)}M`);
    }
