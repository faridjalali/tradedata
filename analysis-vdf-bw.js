...
function analyzePhases(allDaily, breakoutDate) {
  ...
  console.log(`  ${'Period'.padEnd(25)} ${'Price'.padStart(15)} ${'NetDelta%'.padStart(10)} ${'NetDelta'.padStart(10)} ${'Absorption'.padStart(11)} ${'RedDays'.padStart(8)} ${'AvgVol'.padStart(8)} ${'CumDelta'.padStart(10)}`);
  ...
  for (const chunk of chunks) {
    ...
    console.log(`  ${(startDate + '→' + endDate).padEnd(25)} ${priceStr.padStart(15)} ${(netDeltaPct.toFixed(1)+'%').padStart(10)} ${((netDelta/1000).toFixed(0)+'K').padStart(10)} ${(absorpPct.toFixed(0)+'%').padStart(11)} ${(redDays+'/'+chunk.length).padStart(8)} ${(avgVol/1000).toFixed(0).padStart(7)}K ${((runningCumDelta/1000).toFixed(0)+'K').padStart(10)}`);
  }
  ...
  for (let i = 10; i < allDaily.length; i++) {
    ...
    if (priceChg > 3 && netDeltaPct < -5) {
      ...
      console.log(`    ${startDate}→${endDate}: price +${priceChg.toFixed(1)}% BUT delta ${netDeltaPct.toFixed(1)}% (${(netDelta/1000).toFixed(0)}K)  — DISTRIBUTION`);
    }
  }
  ...
  for (let i = 10; i < allDaily.length; i++) {
    ...
    if (priceChg < -3 && netDeltaPct > 5) {
      const endDate = window[window.length-1].date;
      const startDate = window[0].date;
      console.log(`    ${startDate}→${endDate}: price ${priceChg.toFixed(1)}% BUT delta +${netDeltaPct.toFixed(1)}% (+${(netDelta/1000).toFixed(0)}K)  — ACCUMULATION`);
    }
  }
}
...
function analyzeBreakoutProximity(allDaily, breakoutDate) {
  ...
  for (let i = 0; i < last50.length; i++) {
    if (last50[i].delta < 0) {
      if (streak === 0) { streakStart = last50[i].date; streakDeltas = []; }
      streak++; streakDeltas.push(last50[i].delta);
    } else {
      if (streak >= 2) {
        ...
        console.log(`    ${streakStart}→${last50[i-1].date}: ${streak} red days, deltas: ${streakDeltas.map(d => (d/1000).toFixed(0)+'K').join(', ')}${label}`);
      }
      streak = 0;
    }
  }
  if (streak >= 2) {
    ...
    const intensifying = streakDeltas.length >= 2 && Math.abs(streakDeltas[streakDeltas.length-1]) > Math.abs(streakDeltas[0]);
    const label = fading ? ' [FADING]' : intensifying ? ' [INTENSIFYING]' : '';
    console.log(`    ${streakStart}→${last50[last50.length-1].date}: ${streak} red days, deltas: ${streakDeltas.map(d => (d/1000).toFixed(0)+'K').join(', ')}${label}`);
  }
  ...
  for (let i = 0; i < last40.length; i++) {
    if (last40[i].delta > 0) { if (streak === 0) streakStart = last40[i].date; streak++; }
    else { if (streak >= 2) console.log(`    ${streakStart}→${last40[i-1].date}: ${streak} consecutive green days`); streak = 0; }
  }
  if (streak >= 2) console.log(`    ${streakStart}→${last40[last40.length-1].date}: ${streak} consecutive green days`);
...
function analyzeRedDeltaAndVolatility(allDaily, breakoutDate) {
  ...
  for (let t = 0; t < 5; t++) {
    ...
    console.log(`  ${labels[t].padEnd(5)} ${(redD.length+'/'+th.length).padStart(7)} ${((mean(redMags)/1000).toFixed(0)+'K').padStart(10)} ${(redNorm.toFixed(1)+'%').padStart(8)} ${deltaStd.toFixed(1).padStart(9)} ${priceRng.toFixed(2).padStart(9)} ${(netDeltaPct.toFixed(1)+'%').padStart(10)} ${(absrpPct.toFixed(0)+'%').padStart(10)}  ${dateRange}`);
  }
...
(async () => {
  ...
  for (const w of allWeeks) {
    ...
    console.log(`    ${w.weekStart}: price $${w.close.toFixed(2).padStart(6)} ${priceDir}  cum∂=${dDir}${(cumDelta/1000).toFixed(0).padStart(6)}K`);
  }
  ...
  if (breakoutIdx > 0 && breakoutIdx < allDaily.length) {
    ...
    for (const w of postWeeks) {
      ...
      console.log(`    ${w.weekStart}: ${priceDir} $${w.open.toFixed(2)}→$${w.close.toFixed(2)}  ∂=${dir}${w.deltaPct.toFixed(2).padStart(7)}%  (${dir}${(w.delta/1000).toFixed(0).padStart(7)}K)  vol=${(w.avgVol/1e6).toFixed(2)}M  cum∂=${(postCumDelta/1000).toFixed(0)}K`);
    }
