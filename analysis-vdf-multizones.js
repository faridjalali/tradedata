  ...
  for (const w of detected) {
    ...
    if (!overlaps) {
      ...
      for (const other of detected) {
        ...
        if (overlapDays / otherSize > 0.5) used.add(other);
      }
    }
  }
...
(async () => {
  ...
  for (const t of TICKERS) {
    ...
    console.log(
      `  Price range: $${priceLow.toFixed(2)} → $${priceHigh.toFixed(2)} ($${closes[0].toFixed(2)} start, $${closes[closes.length - 1].toFixed(2)} end)`,
    );
        ...
        const gated =
          w.reason === 'concordant'
            ...
            : w.reason === 'slope_gate'
              ...
              : '';
        console.log(
          `    ${w.startDate}→${w.endDate} (${w.winSize}d)  score=${w.score.toFixed(4)}  net∂=${w.netDeltaPct?.toFixed(2) || '?'}%  price=${w.overallPriceChange.toFixed(1)}%${gated}`,
        );
      }
      allTickerResults.push({
        symbol: t.symbol,
        breakout: t.breakout,
        zones: [],
        falsePositive: t.breakout === 'FALSE POSITIVE',
      });
      continue;
    }
  ...
  console.log(
    `  ${'Ticker'.padEnd(8)} ${'Brk'.padEnd(16)} ${'Zones'.padEnd(6)} ${'Zone 1'.padEnd(28)} ${'Score'.padEnd(8)} ${'Net∂%'.padEnd(8)} ${'Slope'.padEnd(8)} ${'AccWk'.padEnd(8)} ${'Zone 2'.padEnd(28)} ${'Score2'.padEnd(8)}`,
  );
  ...
  for (const tr of allTickerResults) {
    ...
    if (z1) {
      ...
      console.log(
        `  ${(tr.symbol + tag).padEnd(8)} ${tr.breakout.padEnd(16)} ${String(tr.zones.length).padEnd(6)} ${z1str.padEnd(28)} ${z1.score.toFixed(4).padEnd(8)} ${z1.netDeltaPct.toFixed(2).padEnd(8)} ${z1.deltaSlopeNorm.toFixed(2).padEnd(8)} ${(z1.accumWeeks + '/' + z1.weeks).padEnd(8)} ${z2str.padEnd(28)} ${z2score.padEnd(8)}`,
      );
    } else {
      console.log(
        `  ${(tr.symbol + tag).padEnd(8)} ${tr.breakout.padEnd(16)} ${String(0).padEnd(6)} — NO ZONES DETECTED —`,
      );
    }
    ...
    for (let i = 2; i < tr.zones.length; i++) {
      const z = tr.zones[i];
      console.log(
        `  ${''.padEnd(8)} ${''.padEnd(16)} ${''.padEnd(6)} ${''.padEnd(28)} ${''.padEnd(8)} ${''.padEnd(8)} ${''.padEnd(8)} ${''.padEnd(8)} ${`${z.startDate}→${z.endDate} (${z.winSize}d)`.padEnd(28)} ${z.score.toFixed(4).padEnd(8)}`,
      );
    }
  }
  ...
  for (const z of tpZones) {
    console.log(
      `    ${z.score.toFixed(4)}  ${z.startDate}→${z.endDate} (${z.winSize}d)  net∂=${z.netDeltaPct.toFixed(2)}%  slope=${z.deltaSlopeNorm.toFixed(2)}  accWk=${z.accumWeeks}/${z.weeks}  absorb=${z.absorptionPct.toFixed(1)}%`,
    );
  }
  ...
  if (tpZones.length > 0) {
    ...
    const weights = [0.25, 0.22, 0.15, 0.15, 0.1, 0.08, 0.05];
    for (let i = 0; i < fields.length; i++) {
      ...
      console.log(
        `    ${names[i].padEnd(16)} w=${weights[i].toFixed(2)}  avg=${avg.toFixed(3)}  min=${mn.toFixed(3)}  max=${mx.toFixed(3)}  avgContrib=${contribution.toFixed(4)}`,
      );
    }
  }
