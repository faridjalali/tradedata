...
function scoreAccumulationDivergence(consolBars1m, preBars1m) {
  ...
  if (weeks.length < 2) return { score: 0, detected: false, reason: 'need_2_weeks', weeks: 0, metrics: {}, daily, weeksData: weeks };
  ...
  if (overallPriceChange > 10) return { score: 0, detected: false, reason: 'price_rising', weeks: weeks.length, metrics: { overallPriceChange }, daily, weeksData: weeks };
  if (overallPriceChange < -45) return { score: 0, detected: false, reason: 'crash', weeks: weeks.length, metrics: { overallPriceChange }, daily, weeksData: weeks };
  ...
  if (netDeltaPct < -1.5) return { score: 0, detected: false, reason: 'concordant_selling', weeks: weeks.length, metrics: { netDeltaPct, overallPriceChange }, daily, weeksData: weeks };
  ...
  return {
    score, detected, reason: detected ? 'accumulation_divergence' : 'below_threshold',
    weeks: weeks.length, accumWeeks, durationMultiplier,
    metrics: { netDeltaPct, deltaSlopeNorm, deltaShift, strongAbsorptionPct, largeBuyVsSell, priceDeltaCorr, accumWeekRatio, overallPriceChange, volContractionScore, s1, s2, s3, s4, s5, s6, s7, s8, rawScore },
    daily, weeksData: weeks
  };
}
...
(async () => {
  ...
  for (const b of recent45) {
    ...
    console.log(`  ${d}  O:${b.open.toFixed(2)}  H:${b.high.toFixed(2)}  L:${b.low.toFixed(2)}  C:${b.close.toFixed(2)}  ${dir}${change}%  Vol:${(b.volume/1000).toFixed(0)}K`);
  }
  console.log();
  ...
  console.log(`Consolidation window: ${consolBars.length} bars (${new Date(consolCutoff * 1000).toISOString().split('T')[0]} to ${new Date(latestTime * 1000).toISOString().split('T')[0]})`);
  ...
  console.log(`  s8 (Vol contraction, 10%):${m.s8?.toFixed(4)}  weighted: ${(m.s8 * 0.10).toFixed(4)}`);
  ...
  console.log(`  ${result.score >= 0.30 ? 'ABOVE THRESHOLD → DETECTED' : 'BELOW THRESHOLD → NOT DETECTED'}`);
  console.log();
  ...
  if (result.daily) {
    ...
    for (const d of result.daily) {
      ...
      console.log(`  ${d.date}  Price:${d.close.toFixed(2)} ${priceDir}${priceChg}%  Delta:${dir}${(d.delta/1000).toFixed(0)}K (${dir}${pct}%)  Vol:${(d.totalVol/1000).toFixed(0)}K`);
    }
    console.log();
  }
  ...
  if (result.weeksData) {
    ...
    for (const w of result.weeksData) {
      const dir = w.delta >= 0 ? '+' : '';
      console.log(`  Week of ${w.weekStart}: Delta ${dir}${(w.delta/1000).toFixed(0)}K (${dir}${w.deltaPct.toFixed(2)}%)  Vol:${(w.totalVol/1000).toFixed(0)}K  Days:${w.nDays}`);
    }
    console.log();
  }
  ...
  if (result.daily) {
    ...
    for (const d of result.daily) {
      cumDelta += d.delta;
      const bar = d.delta >= 0 ? '█'.repeat(Math.min(40, Math.round(Math.abs(d.delta) / (result.daily.reduce((s,x) => Math.max(s, Math.abs(x.delta)), 1) / 20)))) : '';
      const barNeg = d.delta < 0 ? '░'.repeat(Math.min(40, Math.round(Math.abs(d.delta) / (result.daily.reduce((s,x) => Math.max(s, Math.abs(x.delta)), 1) / 20)))) : '';
      console.log(`  ${d.date}  cumΔ:${(cumDelta/1e6).toFixed(2)}M  ${d.delta >= 0 ? '+' : ''}${(d.delta/1000).toFixed(0)}K ${bar}${barNeg}`);
    }
    console.log();
  }
