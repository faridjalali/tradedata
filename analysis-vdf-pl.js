...
function analyzeBreakoutProximity(allDaily, breakoutDate) {
  ...
  console.log(
    `  ${'EndDate'.padEnd(12)} ${'Net∂%'.padStart(6)} ${'Absrp%'.padStart(6)} ${'RedMag'.padStart(6)} ${'RedCnt'.padStart(6)} ${'DeltaStd'.padStart(8)} ${'PriceRng'.padStart(8)} ${'CumDelta'.padStart(9)} ${'VolChg'.padStart(7)}`,
  );
  ...
  for (let endIdx = Math.max(10, n - 60); endIdx <= n; endIdx += 5) {
    ...
    const netDeltaPct =
      mean(window.map((d) => d.totalVol)) > 0
        ? (window.reduce((s, d) => s + d.delta, 0) / window.reduce((s, d) => s + d.totalVol, 0)) * 100
        : 0;
    ...
    const redMagNorm =
      redDays.length > 0 && avgVol > 0 ? (mean(redDays.map((d) => Math.abs(d.delta))) / avgVol) * 100 : 0;
    ...
    const volChg =
      prevWindow.length > 0
        ? ((mean(window.map((d) => d.totalVol)) - mean(prevWindow.map((d) => d.totalVol))) /
            mean(prevWindow.map((d) => d.totalVol))) *
          100
        : 0;
    ...
    console.log(
      `  ${endDate.padEnd(12)} ${netDeltaPct.toFixed(1).padStart(6)} ${absrpPct.toFixed(0).padStart(5)}% ${redMagNorm.toFixed(1).padStart(5)}% ${redDays.length.toString().padStart(6)} ${deltaStd.toFixed(1).padStart(8)} ${priceRange.toFixed(2).padStart(8)} ${(cumDelta / 1000).toFixed(0).padStart(8)}K ${(volChg > 0 ? '+' : '') + volChg.toFixed(0) + '%'} ${marker}`,
    );
  }
  ...
  for (let i = Math.max(5, n - 30); i < n; i++) {
    ...
    const bar =
      delta5d > 0
        ? '+'.repeat(Math.min(25, Math.round(Math.abs(delta5d) / 1000 / 10)))
        : '-'.repeat(Math.min(25, Math.round(Math.abs(delta5d) / 1000 / 10)));
    console.log(
      `    [${String(daysLeft).padStart(2)}d] ${preBreakout[i].date}: 5d∂=${(delta5d / 1000).toFixed(0).padStart(6)}K  accel=${accel.toFixed(0).padStart(5)}%  |${bar}|`,
    );
  }
  ...
  for (const d of last30) {
    if (d.totalVol > avgVol * 1.5) {
      ...
      console.log(
        `    ${d.date}: vol=${(d.totalVol / 1e6).toFixed(2)}M (${mult}x avg)  ∂=${dir}${(d.delta / 1000).toFixed(0)}K  price=$${d.close.toFixed(2)}`,
      );
    }
  }
}
...
function analyzeRedDeltaAndVolatility(allDaily, breakoutDate) {
  ...
  const thirds = [
    preBreakout.slice(0, thirdSize),
    preBreakout.slice(thirdSize, 2 * thirdSize),
    preBreakout.slice(2 * thirdSize),
  ];
  const labels = ['Early', 'Mid', 'Late'];

  console.log(`\n  By thirds (${thirdSize}d each):`);
  console.log(
    `  ${''.padEnd(7)} ${'RedDays'.padStart(7)} ${'AvgRedMag'.padStart(10)} ${'RedNorm'.padStart(8)} ${'DeltaStd'.padStart(9)} ${'PriceRng'.padStart(9)} ${'NetDelta%'.padStart(10)} ${'Absorption'.padStart(10)}`,
  );
  console.log(`  ${'─'.repeat(78)}`);
  ...
  for (let t = 0; t < 3; t++) {
    ...
    console.log(
      `  ${labels[t].padEnd(7)} ${(redD.length + '/' + th.length).padStart(7)} ${((mean(redMags) / 1000).toFixed(0) + 'K').padStart(10)} ${(redNorm.toFixed(1) + '%').padStart(8)} ${deltaStd.toFixed(1).padStart(9)} ${priceRng.toFixed(2).padStart(9)} ${(netDeltaPct.toFixed(1) + '%').padStart(10)} ${(absrpPct.toFixed(0) + '%').padStart(10)}`,
    );
  }
  ...
  console.log(
    `  Price range: 1st half avg=${firstPriceRange.toFixed(2)}%, 2nd half avg=${secondPriceRange.toFixed(2)}%, change=${pvChange > 0 ? '+' : ''}${pvChange.toFixed(0)}%`,
  );
}
