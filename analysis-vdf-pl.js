/**
 * VDF Deep Analysis — PL (Planet Labs)
 * Full period: 2/10/25 - 7/9/25
 * Breakout: 6/5/25
 * Focus: Can we predict proximity to breakout? Characterize accumulation type.
 */
...
/**
 * BREAKOUT PROXIMITY ANALYSIS
 * Can we detect that breakout is imminent? Examine metrics in sliding windows
 * leading up to the breakout to find signals that intensify.
 */
function analyzeBreakoutProximity(allDaily, breakoutDate) {
  console.log(`\n${'='.repeat(80)}`);
  console.log('  BREAKOUT PROXIMITY ANALYSIS');
  console.log(`  Can we detect approaching breakout? (breakout: ${breakoutDate})`);
  console.log('='.repeat(80));

  const breakoutIdx = allDaily.findIndex(d => d.date >= breakoutDate);
  if (breakoutIdx < 20) { console.log('  Insufficient pre-breakout data'); return; }
  const preBreakout = allDaily.slice(0, breakoutIdx);
  const n = preBreakout.length;
  const avgVol = mean(preBreakout.map(d => d.totalVol));

  // Sliding 10-day window metrics approaching breakout
  console.log(`\n  10-day sliding window metrics approaching breakout:`);
  console.log(`  (every 5 days, ending at breakout)`);
  console.log(`  ${'EndDate'.padEnd(12)} ${'Net∂%'.padStart(6)} ${'Absrp%'.padStart(6)} ${'RedMag'.padStart(6)} ${'RedCnt'.padStart(6)} ${'DeltaStd'.padStart(8)} ${'PriceRng'.padStart(8)} ${'CumDelta'.padStart(9)} ${'VolChg'.padStart(7)}`);
  console.log(`  ${'─'.repeat(78)}`);

  const windowMetrics = [];
  for (let endIdx = Math.max(10, n - 60); endIdx <= n; endIdx += 5) {
    const startIdx = Math.max(0, endIdx - 10);
    const window = preBreakout.slice(startIdx, endIdx);
    if (window.length < 5) continue;

    const wn = window.length;
    const netDeltaPct = mean(window.map(d => d.totalVol)) > 0
      ? (window.reduce((s, d) => s + d.delta, 0) / window.reduce((s, d) => s + d.totalVol, 0) * 100) : 0;

    let absrp = 0;
    for (let i = 1; i < wn; i++) {
      if (window[i].close < window[i-1].close && window[i].delta > 0) absrp++;
    }
    const absrpPct = wn > 1 ? absrp / (wn - 1) * 100 : 0;

    const redDays = window.filter(d => d.delta < 0);
    const redMagNorm = redDays.length > 0 && avgVol > 0
      ? mean(redDays.map(d => Math.abs(d.delta))) / avgVol * 100 : 0;

    const deltaStd = std(window.map(d => d.deltaPct));
    const priceRange = mean(window.map(d => d.rangePct));
    const cumDelta = window.reduce((s, d) => s + d.delta, 0);

    const prevWindow = preBreakout.slice(Math.max(0, startIdx - 10), startIdx);
    const volChg = prevWindow.length > 0
      ? ((mean(window.map(d => d.totalVol)) - mean(prevWindow.map(d => d.totalVol))) / mean(prevWindow.map(d => d.totalVol)) * 100) : 0;

    const endDate = window[wn - 1].date;
    const daysToBreakout = breakoutIdx - endIdx;

    windowMetrics.push({ endDate, daysToBreakout, netDeltaPct, absrpPct, redMagNorm, redCount: redDays.length, deltaStd, priceRange, cumDelta, volChg });

    const marker = daysToBreakout <= 5 ? ' ◀◀' : daysToBreakout <= 10 ? ' ◀' : '';
    console.log(`  ${endDate.padEnd(12)} ${netDeltaPct.toFixed(1).padStart(6)} ${absrpPct.toFixed(0).padStart(5)}% ${redMagNorm.toFixed(1).padStart(5)}% ${redDays.length.toString().padStart(6)} ${deltaStd.toFixed(1).padStart(8)} ${priceRange.toFixed(2).padStart(8)} ${(cumDelta/1000).toFixed(0).padStart(8)}K ${(volChg > 0 ? '+' : '')+volChg.toFixed(0)+'%'} ${marker}`);
  }

  // Day-by-day last 20 days before breakout
  console.log(`\n  LAST 20 DAYS BEFORE BREAKOUT (day-by-day):`);
  const last20 = preBreakout.slice(-20);
  let cumD = 0;
  for (let i = 0; i < last20.length; i++) {
    const d = last20[i];
    const prev = i > 0 ? last20[i-1] : null;
    const priceChg = prev ? ((d.close - prev.close) / prev.close * 100).toFixed(1) : '—';
    const dir = d.delta >= 0 ? '+' : '';
    cumD += d.delta;
    const absorb = (prev && d.close < prev.close && d.delta > 0) ? ' ★ABSORB' : '';
    const daysLeft = 20 - i;
    console.log(`    [${String(daysLeft).padStart(2)}d] ${d.date}: $${d.close.toFixed(2).padStart(7)} (${priceChg.padStart(6)}%)  ∂=${dir}${(d.delta/1000).toFixed(0).padStart(6)}K  (${d.deltaPct.toFixed(1).padStart(6)}%)  vol=${(d.totalVol/1e6).toFixed(2)}M  cum∂=${(cumD/1000).toFixed(0)}K${absorb}`);
  }

  // Rolling cumulative delta acceleration (rate of cum delta growth)
  console.log(`\n  CUMULATIVE DELTA ACCELERATION (rolling 5-day cum delta growth):`);
  let runningCumDelta = 0;
  const cumDeltas = [];
  for (const d of preBreakout) {
    runningCumDelta += d.delta;
    cumDeltas.push(runningCumDelta);
  }
  for (let i = Math.max(5, n - 30); i < n; i++) {
    const delta5d = cumDeltas[i] - cumDeltas[i - 5];
    const prevDelta5d = i >= 10 ? cumDeltas[i - 5] - cumDeltas[i - 10] : 0;
    const accel = prevDelta5d !== 0 ? ((delta5d - prevDelta5d) / Math.abs(prevDelta5d) * 100) : 0;
    const daysLeft = n - i;
    const bar = delta5d > 0 ? '+'.repeat(Math.min(25, Math.round(Math.abs(delta5d)/1000/10))) : '-'.repeat(Math.min(25, Math.round(Math.abs(delta5d)/1000/10)));
    console.log(`    [${String(daysLeft).padStart(2)}d] ${preBreakout[i].date}: 5d∂=${(delta5d/1000).toFixed(0).padStart(6)}K  accel=${accel.toFixed(0).padStart(5)}%  |${bar}|`);
  }

  // Consecutive green delta streaks near breakout
  console.log(`\n  CONSECUTIVE GREEN DELTA STREAKS (last 30 days):`);
  ...
  for (let i = 0; i < last30.length; i++) {
    if (last30[i].delta > 0) {
      ...
      if (streak >= 2) {
        console.log(`    ${streakStart}→${last30[i-1].date}: ${streak} consecutive green days`);
      }
      streak = 0;
    }
  }
  if (streak >= 2) console.log(`    ${streakStart}→${last30[last30.length-1].date}: ${streak} consecutive green days`);

  // Volume surge detection near breakout
  console.log(`\n  VOLUME SURGE DETECTION (days where vol > 1.5x avg, last 30 days):`);
  for (const d of last30) {
    if (d.totalVol > avgVol * 1.5) {
      const dir = d.delta >= 0 ? '+' : '';
      const mult = (d.totalVol / avgVol).toFixed(1);
      console.log(`    ${d.date}: vol=${(d.totalVol/1e6).toFixed(2)}M (${mult}x avg)  ∂=${dir}${(d.delta/1000).toFixed(0)}K  price=$${d.close.toFixed(2)}`);
    }
  }
}

/**
 * RED DELTA CONTRACTION + DELTA VOLATILITY ANALYSIS
 */
function analyzeRedDeltaAndVolatility(allDaily, breakoutDate) {
  ...
  console.log(`\n  By thirds (${thirdSize}d each):`);
  console.log(`  ${''.padEnd(7)} ${'RedDays'.padStart(7)} ${'AvgRedMag'.padStart(10)} ${'RedNorm'.padStart(8)} ${'DeltaStd'.padStart(9)} ${'PriceRng'.padStart(9)} ${'NetDelta%'.padStart(10)} ${'Absorption'.padStart(10)}`);
  console.log(`  ${'─'.repeat(78)}`);
  ...
  for (let t = 0; t < 3; t++) {
    ...
    const netDeltaPct = th.reduce((s,d) => s+d.totalVol,0) > 0
      ? th.reduce((s,d) => s+d.delta,0) / th.reduce((s,d) => s+d.totalVol,0) * 100 : 0;
    let absrp = 0;
    ...
    console.log(`  ${labels[t].padEnd(7)} ${(redD.length+'/'+th.length).padStart(7)} ${((mean(redMags)/1000).toFixed(0)+'K').padStart(10)} ${(redNorm.toFixed(1)+'%').padStart(8)} ${deltaStd.toFixed(1).padStart(9)} ${priceRng.toFixed(2).padStart(9)} ${(netDeltaPct.toFixed(1)+'%').padStart(10)} ${(absrpPct.toFixed(0)+'%').padStart(10)}`);
  }

  // Linear regression of |red delta|
  ...
  console.log(`  Delta volatility: 1st half std=${firstDeltaStd.toFixed(1)}, 2nd half std=${secondDeltaStd.toFixed(1)}, change=${dvChange > 0 ? '+' : ''}${dvChange.toFixed(0)}%`);

  // Price volatility trend
  const firstPriceRange = mean(preBreakout.slice(0, halfIdx).map(d => d.rangePct));
  const secondPriceRange = mean(preBreakout.slice(halfIdx).map(d => d.rangePct));
  const pvChange = firstPriceRange > 0 ? ((secondPriceRange - firstPriceRange) / firstPriceRange * 100) : 0;
  console.log(`  Price range: 1st half avg=${firstPriceRange.toFixed(2)}%, 2nd half avg=${secondPriceRange.toFixed(2)}%, change=${pvChange > 0 ? '+' : ''}${pvChange.toFixed(0)}%`);
}

(async () => {
  console.log('=== VDF Deep Analysis — PL (Planet Labs) ===\n');

  console.log('Fetching full period (2/10/25 → 7/9/25)...');
  const allBars = await fetch1mChunked('PL', '2025-02-10', '2025-07-09');
  console.log('Fetching pre-context (1/10/25 → 2/10/25)...');
  const preBars = await fetch1mChunked('PL', '2025-01-10', '2025-02-10');
  ...
  console.log('='.repeat(80));
  console.log('  PL — FULL PERIOD OVERVIEW (2/10/25 → 7/9/25)');
  console.log('='.repeat(80));
  ...
  // Subwindow scanning
  console.log(`\n${'='.repeat(80)}`);
  console.log('  PL — SUBWINDOW SCANNING');
  console.log('='.repeat(80));
  ...
  if (zones.length > 0) {
    console.log(`\n${'='.repeat(80)}`);
    console.log(`  PL — ${zones.length} ACCUMULATION ZONE(S) DETECTED`);
    console.log('='.repeat(80));
  ...
  // RED DELTA & DELTA VOLATILITY ANALYSIS
  analyzeRedDeltaAndVolatility(allDaily, '2025-06-05');

  // BREAKOUT PROXIMITY ANALYSIS
  analyzeBreakoutProximity(allDaily, '2025-06-05');

  // Breakout and post-breakout
  const breakoutIdx = allDaily.findIndex(d => d.date >= '2025-06-05');
  if (breakoutIdx > 0 && breakoutIdx < allDaily.length) {
    console.log(`\n${'='.repeat(80)}`);
    console.log('  PL — BREAKOUT & POST-BREAKOUT');
    console.log('='.repeat(80));
    const postSlice = allDaily.slice(breakoutIdx, Math.min(breakoutIdx + 20, allDaily.length));
    const preBreakout = allDaily.slice(0, breakoutIdx);
    for (let i = 0; i < postSlice.length; i++) {
      const d = postSlice[i];
      const prev = i > 0 ? postSlice[i-1] : preBreakout[preBreakout.length - 1];
      const priceChg = prev ? ((d.close - prev.close) / prev.close * 100).toFixed(1) : '—';
      const dir = d.delta >= 0 ? '+' : '';
