/**
 * VDF Deep Analysis — BE (Bloom Energy)
 * Full period: 8/15/24 - 11/26/24
 */
...
function findAccumulationZones(allDaily, preDaily) {
  ...
  // Greedy clustering with minimum 10 trading day gap
  const zones = [];
  for (const w of detected) {
    ...
    for (const z of zones) {
      ...
      // Also require minimum 10-day gap between non-overlapping zones
...
(async () => {
  console.log('=== VDF Deep Analysis — BE ===\n');

  console.log('Fetching full period (8/15/24 → 11/26/24)...');
  const allBars = await fetch1mChunked('BE', '2024-08-15', '2024-11-26');
  console.log('Fetching pre-context (7/15/24 → 8/15/24)...');
  const preBars = await fetch1mChunked('BE', '2024-07-15', '2024-08-15');
  ...
  console.log('  BE — FULL PERIOD OVERVIEW (8/15/24 → 11/26/24)');
  console.log('='.repeat(80));
  ...
  // Subwindow scanning
  console.log(`\n${'='.repeat(80)}`);
  console.log('  BE — SUBWINDOW SCANNING');
  console.log('='.repeat(80));
  ...
  if (zones.length > 0) {
    console.log(`\n${'='.repeat(80)}`);
    console.log(`  BE — ${zones.length} ACCUMULATION ZONE(S) DETECTED`);
    console.log('='.repeat(80));
  ...
  // Pre-breakout analysis (last 10 days of period)
  console.log(`\n${'='.repeat(80)}`);
  console.log('  BE — PRE-BREAKOUT (last 10 days of period)');
  console.log('='.repeat(80));

  const preBreakout = allDaily.slice(-10);
