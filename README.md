...
- Node.js 22+
- PostgreSQL (primary database required; optional second database for divergence data)
...
# Edit .env with your DATABASE_URL, DATA_API_KEY, etc.
...
| **Database** | PostgreSQL (pg), dual pool support |
...
Only created when `DIVERGENCE_DATABASE_URL` is configured.
...
| Variable | Default | Description |
|---|---|---|
| `DIVERGENCE_DATABASE_URL` | | Separate PostgreSQL for divergence data |
...
6. Initialize primary database (create tables if needed)
7. Initialize divergence database (if configured)
8. Load trading calendar
9. Start alert pruning timer
10. Schedule next divergence scan
11. Begin listening on configured port
...
- **Readiness:** `GET /readyz` — Checks primary DB connectivity and divergence pool status. Use for load balancer routing.
...
**Required environment variables:** `DATABASE_URL`, `DATA_API_KEY`
...
For the optional divergence database, set `DIVERGENCE_DATABASE_URL` to a second PostgreSQL connection string. Both databases can be the same instance with different databases, or separate instances.
