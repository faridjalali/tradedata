...
const STATIC_INDEX_MAP: Record<BreadthIndex, string[]> = {
...
const runtimeOverrideMap = new Map<BreadthIndex, string[]>();
...
export function setBreadthConstituentOverrides(overrides: Partial<Record<BreadthIndex, string[]>>): void {
  runtimeOverrideMap.clear();
  ...
  for (const [index, tickers] of entries) {
    ...
    const normalized = normalizeTickerList(Array.isArray(tickers) ? tickers : []);
    if (normalized.length === 0) continue;
    runtimeOverrideMap.set(index, normalized);
  }
}

export function clearBreadthConstituentOverrides(): void {
  runtimeOverrideMap.clear();
}

export function getBreadthConstituentOverrides(): Partial<Record<BreadthIndex, string[]>> {
  const out: Partial<Record<BreadthIndex, string[]>> = {};
  for (const [index, tickers] of runtimeOverrideMap.entries()) {
    out[index] = [...tickers];
  }
  return out;
}
...
export function getConstituentsForIndex(index: BreadthIndex): string[] {
  const override = runtimeOverrideMap.get(index);
  if (override && override.length > 0) return override;
  return STATIC_INDEX_MAP[index] ?? [];
}
...
export function getAllBreadthTickers(): Set<string> {
  const union = new Set<string>();
  for (const index of ALL_BREADTH_INDICES) {
    const constituents = getConstituentsForIndex(index);
    for (const ticker of constituents) union.add(ticker);
  }
  return union;
}

export function getBreadthConstituentCounts(): Record<BreadthIndex, number> {
  const out = {} as Record<BreadthIndex, number>;
  for (const index of ALL_BREADTH_INDICES) {
    out[index] = getConstituentsForIndex(index).length;
  }
  return out;
}
