...
const booleanInput = z.union([z.boolean(), z.number(), z.string()]).transform((val) => {
...
const scaleTime = z.union([z.number().finite(), z.string().min(1)]);
...
const tickerQueryParam = z
  .object({
    ticker: tickerSymbol,
  })
  .passthrough();

const tickerListQueryParams = z
  .object({
...
const divergenceSummaryQueryParams = z
  .object({
    ticker: z.string().optional(),
    tickers: z.string().optional(),
