...
// Uses 1-minute volume delta to reveal net buying that diverges from price.
// Weekly-smoothed, duration-scaling, aberration-resistant.
//
// See ALGORITHM-VD-ACCUMULATION.md for full documentation.

/**
 * Aggregate 1-minute bars into ISO-week buckets.
 */
...
function scoreAccumulationDivergence(consolBars1m, preBars1m) {
  ...
  const s2 = Math.max(0, Math.min(1, (deltaSlopeNorm + 0.5) / 4));       // Delta Slope (20%)
  const s3 = Math.max(0, Math.min(1, (deltaShift + 1) / 8));             // Delta Shift (15%)
  const s4 = Math.max(0, Math.min(1, strongAbsorptionPct / 18));          // Absorption (15%)
  const s5 = Math.max(0, Math.min(1, (largeBuyVsSell + 3) / 12));        // Buy vs Sell (10%)
  const s6 = Math.max(0, Math.min(1, (-priceDeltaCorr + 0.3) / 1.5));    // Anti-corr (10%)
  const s7 = Math.max(0, Math.min(1, (accumWeekRatio - 0.2) / 0.6));     // Week ratio (5%)

  const rawScore = s1 * 0.25 + s2 * 0.20 + s3 * 0.15 + s4 * 0.15 + s5 * 0.10 + s6 * 0.10 + s7 * 0.05;

  // Duration scaling: 70% at 2 weeks → 100% at 6+ weeks
  const durationMultiplier = Math.min(1.0, 0.70 + (weeks.length - 2) * 0.075);
  ...
  return {
    ...
    weeks: weeks.length,
    accumWeeks,
    metrics: { netDeltaPct, deltaSlopeNorm, deltaShift, strongAbsorptionPct, largeBuyVsSell, priceDeltaCorr, accumWeekRatio, overallPriceChange, s1, s2, s3, s4, s5, s6, s7 },
  };
}

/**
 * Run VD Accumulation Divergence detection for a ticker.
 * Fetches 6 weeks of 1m data + 30-day pre-context using the provided data fetcher.
 *
 * @param {string} ticker
 * @param {object} options
 * @param {function} options.dataApiFetcher — async (symbol, interval, lookbackDays, opts) => bars[]
 * @param {AbortSignal|null} options.signal
 * @returns {Promise<object>} VD accumulation result
 */
async function detectVDAccumulation(ticker, options) {
...
module.exports = { detectHTF, HTF_CONFIG, HTF_CONFIG_MODERATE, detectVDAccumulation, scoreAccumulationDivergence };
