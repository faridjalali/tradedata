...
async function fetch15mChunked(symbol, from, to) {
  ...
  while (cursor < end) {
    const cEnd = new Date(cursor); cEnd.setDate(cEnd.getDate() + 120);
...
function rollingStd(arr, w) {
  ...
  for (let i = w - 1; i < arr.length; i++) {
    let s = 0, s2 = 0;
    for (let j = i - w + 1; j <= i; j++) { s += arr[j]; s2 += arr[j] ** 2; }
    const m = s / w;
...
function pctRank(val, sorted) {
  ...
  let c = 0;
  for (const v of sorted) { if (v < val) c++; }
...
const KNOWN_EPISODES = {
  RKLB: [
    ...
    { label: 'Sep-Nov 2024 run → Dec consol', impulseFrom: '2024-09-06', impulseTo: '2024-11-29', consolTo: '2025-01-21' },
    // Episode: Jan 2025 rally → Feb-Mar pullback → Apr breakout
    { label: 'Dec 2024 dip-buy → Jan high → Feb consol', impulseFrom: '2024-12-09', impulseTo: '2025-01-24', consolTo: '2025-03-10' },
    // Episode: Apr-Jul 2025 rally → Jul-Sep consolidation → Oct breakout
    { label: 'Apr-Jul 2025 rally → Jul-Sep consol', impulseFrom: '2025-04-22', impulseTo: '2025-07-17', consolTo: '2025-10-03' },
    // Episode: Oct-Jan 2026 rally → Jan-Feb consolidation (current)
    { label: 'Nov-Jan 2026 rally → Jan-Feb consol', impulseFrom: '2025-11-21', impulseTo: '2026-01-16', consolTo: '2026-02-14' },
  ],
  ASTS: [
    // ASTS similar pattern — massive run then consolidation
    { label: 'May-Sep 2024 run → Oct consol', impulseFrom: '2024-05-01', impulseTo: '2024-09-20', consolTo: '2024-11-15' },
    { label: 'Nov 2024 run → Dec consol', impulseFrom: '2024-11-01', impulseTo: '2024-12-13', consolTo: '2025-01-15' },
    { label: 'Mar-Jun 2025 run → Jun-Aug consol', impulseFrom: '2025-03-01', impulseTo: '2025-06-15', consolTo: '2025-08-30' },
    { label: 'Sep-Nov 2025 run → Dec consol', impulseFrom: '2025-09-01', impulseTo: '2025-11-15', consolTo: '2026-01-15' },
  ]
};
...
function analyzeDailyConsolidation(daily, impulseFromDate, impulseToDate, consolToDate) {
  ...
  let consolLow = Infinity, consolHigh = -Infinity;
  ...
  if (third >= 1) {
    ...
    var firstThirdLow = NaN, midThirdLow = NaN, lastThirdLow = NaN, higherLows = false;
  }
  ...
  const impulseVol = Math.sqrt(impulseReturns.reduce((s, v) => s + (v - impulseAvgRet) ** 2, 0) / impulseReturns.length);
  ...
  const bbWidthStart = bbWidths.length >= 4 ? bbWidths.slice(0, Math.floor(bbWidths.length / 4)).reduce((s, v) => s + v, 0) / Math.floor(bbWidths.length / 4) : NaN;
  const bbWidthEnd = bbWidths.length >= 4 ? bbWidths.slice(-Math.floor(bbWidths.length / 4)).reduce((s, v) => s + v, 0) / Math.floor(bbWidths.length / 4) : NaN;
  ...
  return {
    ...
    higherLows: {
      firstThirdLow, midThirdLow, lastThirdLow, higherLows
    },
    dailyRange: {
      slope: rangeSlope,
      r2: rangeR2,
...
function analyzeIntraday(bars15m, bars1m, daily, impulseToDate, consolToDate) {
  ...
  const netDeltaPct = (totalBuyVol + totalSellVol) > 0
    ? ((totalBuyVol - totalSellVol) / (totalBuyVol + totalSellVol)) * 100
    : 0;
  ...
  let deltaTrendSlope = 0, deltaTrendR2 = 0;
  if (dailyDeltas.length >= 5) {
    const dr = linReg(dailyDeltas.map((_, i) => i), dailyDeltas);
  ...
  for (const b of consol15m) {
    ...
    cumVP += tp * b.volume; cumV += b.volume;
...
async function main() {
  ...
  for (const symbol of ['RKLB', 'ASTS']) {
    ...
    for (const ep of episodes) {
      ...
      console.log(`\n  DAILY METRICS:`);
      console.log(`    Impulse: ${dailyMetrics.impulseDays} days, +${dailyMetrics.impulseGainPct.toFixed(1)}%, $${dailyMetrics.impulseLow.toFixed(2)}→$${dailyMetrics.impulseHigh.toFixed(2)}`);
      console.log(`    Consol: ${dailyMetrics.consolDays} days, retrace ${dailyMetrics.retracePct.toFixed(1)}%, range ${dailyMetrics.consolRangePct.toFixed(1)}%`);
      console.log(`    Volume: impulse avg ${dailyMetrics.volume.avgImpulseVol.toLocaleString()}, consol avg ${dailyMetrics.volume.avgConsolVol.toLocaleString()}, ratio ${dailyMetrics.volume.declineRatio.toFixed(3)}`);
      console.log(`    Volume slope: ${dailyMetrics.volume.slopeDecay.toFixed(6)} (R²=${dailyMetrics.volume.slopeR2.toFixed(3)}) ${dailyMetrics.volume.slopeDecay < 0 ? '↓ declining' : '↑ increasing'}`);
      console.log(`    Higher lows: ${dailyMetrics.higherLows.higherLows ? 'YES' : 'NO'} (thirds: $${dailyMetrics.higherLows.firstThirdLow?.toFixed(2)}, $${dailyMetrics.higherLows.midThirdLow?.toFixed(2)}, $${dailyMetrics.higherLows.lastThirdLow?.toFixed(2)})`);
      console.log(`    Daily range decay: slope=${dailyMetrics.dailyRange.slope.toFixed(6)}, R²=${dailyMetrics.dailyRange.r2.toFixed(3)}, contraction=${dailyMetrics.dailyRange.contractionRatio?.toFixed(3) || 'N/A'}`);
      console.log(`    Price near high (within 15%): ${dailyMetrics.nearHighPct.toFixed(1)}% of consol days`);
      console.log(`    Volatility: daily=${(dailyMetrics.volatility.dailyVol * 100).toFixed(2)}%, impulse=${(dailyMetrics.volatility.impulseVol * 100).toFixed(2)}%, ratio=${dailyMetrics.volatility.contractionRatio?.toFixed(3) || 'N/A'}`);
      console.log(`    Bollinger Width: start=${dailyMetrics.bollingerBand.widthStart?.toFixed(4) || 'N/A'}, end=${dailyMetrics.bollingerBand.widthEnd?.toFixed(4) || 'N/A'}, contraction=${dailyMetrics.bollingerBand.contraction?.toFixed(3) || 'N/A'}`);
      ...
      if (intradayMetrics && !intradayMetrics.error) {
        ...
        console.log(`    15m bar range ratio (consol/pre): ${intradayMetrics.barRangeRatio?.toFixed(3) || 'N/A'} (avg consol bar range: ${intradayMetrics.avgConsolBarRangePct.toFixed(3)}%)`);
        console.log(`    Morning/afternoon vol ratio: ${intradayMetrics.morningToAfternoonRatio?.toFixed(3) || 'N/A'}`);
        console.log(`    Net delta (buy-sell): ${intradayMetrics.netDeltaPct.toFixed(2)}% ${intradayMetrics.netDeltaPct > 0 ? '(net buying)' : '(net selling)'}`);
        console.log(`    Delta trend: slope=${intradayMetrics.deltaTrend.slope.toFixed(0)}, R²=${intradayMetrics.deltaTrend.r2.toFixed(3)} (${intradayMetrics.deltaTrend.nDays} days)`);
        console.log(`    VWAP: avg dev ${intradayMetrics.vwap.avgDevPct.toFixed(3)}%, convergence ${intradayMetrics.vwap.convergence?.toFixed(3) || 'N/A'}`);
    ...
    const allMetrics = episodes.map(ep => analyzeDailyConsolidation(daily, ep.impulseFrom, ep.impulseTo, ep.consolTo)).filter(Boolean);
    if (allMetrics.length > 0) {
      ...
      console.log(`    Vol declining in: ${allMetrics.filter(m => m.volume.slopeDecay < 0).length}/${allMetrics.length} episodes`);
      console.log(`    Higher lows in: ${allMetrics.filter(m => m.higherLows.higherLows).length}/${allMetrics.length} episodes`);
      console.log(`    Avg near-high%: ${avg(allMetrics.map(m => m.nearHighPct)).toFixed(1)}%`);
      console.log(`    Avg vol contraction: ${avg(validVals(allMetrics.map(m => m.volatility.contractionRatio))).toFixed(3)}`);
      console.log(`    Avg BB contraction: ${avg(validVals(allMetrics.map(m => m.bollingerBand.contraction))).toFixed(3)}`);
      console.log(`    Avg daily range contraction: ${avg(validVals(allMetrics.map(m => m.dailyRange.contractionRatio))).toFixed(3)}`);
    }
  }
