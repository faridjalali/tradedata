...
function extractFeatures(consolBars1m, preContextBars1m) {
  ...
  for (const b of consolBars1m) {
    ...
    if (!dailyMap.has(d)) dailyMap.set(d, { buyVol: 0, sellVol: 0, totalVol: 0, closes: [], opens: [], highs: [], lows: [] });
  ...
  const cumDeltas = [];
  let cd = 0;
  for (const d of deltas) { cd += d; cumDeltas.push(cd); }
  ...
  features.priceDeltaDivRatio = features.priceSlopeNorm !== 0
    ? features.deltaSlopeNorm / Math.abs(features.priceSlopeNorm) : 0;
  ...
  const supportLevel = priceLow + priceRange * 0.30;
  const resistanceLevel = priceLow + priceRange * 0.70;

  let supportDelta = 0, supportVol = 0, supportDays = 0;
  let resistanceDelta = 0, resistanceVol = 0, resistanceDays = 0;
  for (const d of daily) {
    if (d.close <= supportLevel) {
      supportDelta += d.delta; supportVol += d.totalVol; supportDays++;
    }
    if (d.close >= resistanceLevel) {
      resistanceDelta += d.delta; resistanceVol += d.totalVol; resistanceDays++;
    }
  }
  ...
  let absorptionDays = 0, strongAbsorptionDays = 0, distributionDays = 0;
  ...
  if (validRsiIdx.length >= 10) {
    ...
    if (q >= 2) {
      ...
      features.rsiHigherLow = 0; features.priceLowerLow = 0; features.rsiLowShift = 0;
    }
    ...
    features.vdRsiSlope = 0; features.vdRsiPriceSlope = 0; features.vdRsiDivergence = 0;
    features.rsiHigherLow = 0; features.priceLowerLow = 0; features.rsiLowShift = 0;
    features.vdRsiStart = 50; features.vdRsiEnd = 50; features.vdRsiChange = 0;
  }

  // === F25: Volume slope (log-linear) ===
  features.volSlope = linReg(xs, daily.map(d => Math.log(Math.max(1, d.totalVol)))).slope;
  ...
  if (preDates.length > 0) {
    const preAvgDelta = preDates.reduce((s, d) => {
      const day = preDailyMap.get(d);
  ...
  features.largeBuyDayRatio = (daily.filter(d => d.delta > avgDailyVol * 0.10).length / daily.length) * 100;
  features.largeSellDayRatio = (daily.filter(d => d.delta < -avgDailyVol * 0.10).length / daily.length) * 100;
  ...
  if (daily.length >= 10) {
    ...
    features.rolling5DeltaSlope = avgDailyVol > 0 ? (linReg(rolling5.map((_, i) => i), rolling5).slope / avgDailyVol) * 100 : 0;
  } else {
...
const TEST_CASES = [
  ...
  { symbol: 'RKLB', from: '2026-01-06', to: '2026-02-14', preFrom: '2025-12-06', expected: false, label: 'RKLB Jan-Feb 2026 (current, no breakout yet)' },
  { symbol: 'IREN', from: '2026-01-06', to: '2026-02-14', preFrom: '2025-12-06', expected: false, label: 'IREN Jan-Feb 2026 (current, no breakout yet)' },

  // Additional potential negatives — tickers that declined and didn't recover
  { symbol: 'SMCI', from: '2024-10-01', to: '2024-11-15', preFrom: '2024-09-01', expected: false, label: 'SMCI Oct-Nov 2024 (continued decline)' },
...
async function main() {
  ...
  for (const tc of TEST_CASES) {
    ...
    try {
      ...
      console.log(`  Price: $${features.priceStart.toFixed(2)} → $${features.priceEnd.toFixed(2)} (${features.priceChangePct.toFixed(1)}%)`);
      ...
      console.log(`    T1: ${features.t1DeltaPct.toFixed(3)}% | T2: ${features.t2DeltaPct.toFixed(3)}% | T3: ${features.t3DeltaPct.toFixed(3)}%`);
      ...
      console.log(`    Start → End:          ${features.vdRsiStart.toFixed(1)} → ${features.vdRsiEnd.toFixed(1)} (Δ${features.vdRsiChange.toFixed(1)})`);
  ...
  const featureNames = Object.keys(positives[0].features).filter(k =>
    typeof positives[0].features[k] === 'number' && !['priceStart', 'priceEnd', 'nDays', 'supportDays'].includes(k)
  );

  console.log(`${'Feature'.padEnd(28)} ${'Pos Mean'.padStart(10)} ${'Neg Mean'.padStart(10)} ${'Diff'.padStart(10)} ${'Sep?'.padStart(8)} ${'Rating'.padStart(15)}`);
  ...
  for (const fname of featureNames) {
    ...
    let rating = '';
    let ratingScore = 0;
    if (posMin > negMax || negMin > posMax) { rating = '★★★ PERFECT'; ratingScore = 3; }
    else if ((posMean > 0 && negMean <= 0) || (posMean <= 0 && negMean > 0)) { rating = '★★ SIGN-FLIP'; ratingScore = 2; }
    else if (Math.abs(diff) > 0.3 * Math.max(Math.abs(posMean), Math.abs(negMean), 0.01)) { rating = '★ USEFUL'; ratingScore = 1; }

    ranked.push({ fname, posMean, negMean, diff, rating, ratingScore });

    const sep = (posMin > negMax) ? 'FULL' : (negMin > posMax) ? 'FULL-R' : 'OVER';
    console.log(`${fname.padEnd(28)} ${posMean.toFixed(4).padStart(10)} ${negMean.toFixed(4).padStart(10)} ${diff.toFixed(4).padStart(10)} ${sep.padStart(8)} ${rating.padStart(15)}`);
  }
  ...
  for (const r of ranked.filter(r => r.ratingScore > 0)) {
    console.log(`    ${r.rating.padEnd(18)} ${r.fname.padEnd(28)} pos=${r.posMean.toFixed(4)}, neg=${r.negMean.toFixed(4)}`);
  }
  ...
  for (const r of allResults) {
    ...
    scoreB += Math.max(0, Math.min(1, (-f.priceDeltaCorr + 1) / 2)) * 0.20;
    ...
    console.log(`  ${r.label}`);
    console.log(`    Formula A: ${scoreA.toFixed(4)} | Formula B: ${scoreB.toFixed(4)} | Final: ${scoreFinal.toFixed(4)}`);
    console.log(`    Detected: ${detected ? 'YES' : 'NO'} | Expected: ${r.expected ? 'YES' : 'NO'} | ${correct ? '✅' : '⚠️ MISS'}`);
    console.log();
  }

  const accuracy = allResults.filter(r => (Math.max(
    (() => { const f = r.features; let s = 0;
      s += Math.max(0, Math.min(1, (f.deltaVsExpected + 3) / 10)) * 0.20;
      s += Math.max(0, Math.min(1, (f.deltaImprovement + 2) / 6)) * 0.15;
      s += Math.max(0, Math.min(1, (f.halvesImprovement + 2) / 6)) * 0.10;
      s += Math.max(0, Math.min(1, (f.supportDeltaPct + 3) / 8)) * 0.10;
      s += Math.max(0, Math.min(1, (f.absMinusDist + 10) / 25)) * 0.15;
      s += Math.max(0, Math.min(1, (f.stealthAccum) / 5)) * 0.10;
      let rs = 0; if (f.vdRsiDivergence) rs = 0.9; else if (f.rsiHigherLow) rs = 0.7;
      else if (f.vdRsiChange > 0) rs = Math.min(0.6, f.vdRsiChange / 15);
      else if (f.halvesImprovement > 0) rs = 0.25; s += rs * 0.10;
      let ps = 0; if (f.priceChangePct > -45 && f.priceChangePct < 5) {
        if (f.priceChangePct < 0) ps = Math.min(1, Math.abs(f.priceChangePct) / 30); else ps = 0.15; }
      s += ps * 0.10; return s; })(),
    (() => { const f = r.features; let s = 0;
      s += Math.max(0, Math.min(1, (f.priceDeltaDivRatio + 1) / 3)) * 0.25;
      s += Math.max(0, Math.min(1, (-f.priceDeltaCorr + 1) / 2)) * 0.20;
      s += Math.max(0, Math.min(1, (f.netDeltaPct + 2) / 5)) * 0.15;
      s += Math.max(0, Math.min(1, f.absorptionPct / 40)) * 0.15;
      let rs = 0; if (f.vdRsiDivergence) rs = 0.9; else if (f.rsiHigherLow) rs = 0.7;
      else if (f.vdRsiChange > 0) rs = Math.min(0.6, f.vdRsiChange / 15);
      else if (f.halvesImprovement > 0) rs = 0.25; s += rs * 0.15;
      let ps = 0; if (f.priceChangePct > -45 && f.priceChangePct < 5) {
        if (f.priceChangePct < 0) ps = Math.min(1, Math.abs(f.priceChangePct) / 30); else ps = 0.15; }
      s += ps * 0.10; return s; })()
  ) >= 0.35) === r.expected).length;
