/**
 * VDF Deep Analysis — EOSE
 * Full period: 1/21/25 - 5/15/25
 * Accumulation: unknown — algorithm to find it
 * Breakout: unknown — to be determined from data
 */
...
(async () => {
  console.log('=== VDF Deep Analysis — EOSE ===\n');

  console.log('Fetching full period (1/21/25 → 5/15/25)...');
  const allBars = await fetch1mChunked('EOSE', '2025-01-21', '2025-05-15');
  console.log('Fetching pre-context (12/21/24 → 1/21/25)...');
  const preBars = await fetch1mChunked('EOSE', '2024-12-21', '2025-01-21');
  ...
  console.log('  EOSE — FULL PERIOD OVERVIEW (1/21/25 → 5/15/25)');
  console.log('='.repeat(80));
  ...
  // ---- Cumulative delta vs price trajectory ----
  console.log(`\n${'='.repeat(80)}`);
  console.log('  EOSE — CUMULATIVE DELTA vs PRICE TRAJECTORY');
  console.log('='.repeat(80));

  let cumDelta = 0;
  console.log('  (Weekly cumulative delta to spot divergence visually)');
  for (const w of allWeeks) {
    cumDelta += w.delta;
    const priceDir = w.close >= w.open ? '▲' : '▼';
  ...
  // ---- Subwindow scanning ----
  console.log(`\n${'='.repeat(80)}`);
  console.log('  EOSE — SUBWINDOW SCANNING (across full 1/21 → 5/15 period)');
  console.log('='.repeat(80));
  ...
  if (allResults.length > 0) {
    ...
    console.log(`\n${'='.repeat(80)}`);
    console.log('  EOSE — BEST SUBWINDOW DETAIL');
    console.log('='.repeat(80));
    ...
    // Top 10 detected windows overview
    console.log('\n  Top 10 detected windows:');
    for (const w of allResults.slice(0, 10)) {
      console.log(`    ${w.startDate}→${w.endDate} (${w.winSize}d)  score=${w.score.toFixed(4)}  net∂=${w.netDeltaPct.toFixed(2)}%  price=${w.overallPriceChange.toFixed(1)}%  slope=${w.deltaSlopeNorm.toFixed(2)}  accWk=${w.accumWeeks}/${w.weeks}`);
    }

    // ---- Pre/post breakout around end of best window ----
    const breakDate = best.endDate;
    console.log(`\n${'='.repeat(80)}`);
    console.log(`  EOSE — PRE-BREAKOUT (10 days ending ${breakDate})`);
    console.log('='.repeat(80));

    const preBreakout = allDaily.filter(d => d.date <= breakDate).slice(-10);
    ...
    console.log(`\n  POST (after ${breakDate}):`);
    const postBreakout = allDaily.filter(d => d.date > breakDate).slice(0, 15);
