...
(async () => {
  ...
  if (allResults.length > 0) {
    ...
    console.log(`    s5 Buy vs Sell (10%):     ${c.s5.toFixed(4)}  → ${(c.s5 * 0.1).toFixed(4)}`);
    ...
    for (const w of best.weeksData) {
      ...
      console.log(
        `    ${w.weekStart}: ${dir}${(w.effectiveDelta / 1000).toFixed(0)}K  ∂%=${dir}${w.deltaPct.toFixed(2)}%  rng=${w.avgRange.toFixed(2)}%  vol=${(w.avgVol / 1e6).toFixed(2)}M  [${w.nDays}d]`,
      );
    }
    ...
    for (let i = 0; i < bestSlice.length; i++) {
      ...
      console.log(
        `    ${d.date}: $${d.close.toFixed(2).padStart(7)} (${priceChg.padStart(6)}%)  ∂=${dir}${(d.delta / 1000).toFixed(0).padStart(6)}K  (${d.deltaPct.toFixed(1).padStart(6)}%)  rng=${d.rangePct.toFixed(2).padStart(6)}%  vol=${(d.totalVol / 1e6).toFixed(2)}M${absorb}`,
      );
    }
    ...
    for (const w of allResults.slice(0, 10)) {
      console.log(
        `    ${w.startDate}→${w.endDate} (${w.winSize}d)  score=${w.score.toFixed(4)}  net∂=${w.netDeltaPct.toFixed(2)}%  price=${w.overallPriceChange.toFixed(1)}%  slope=${w.deltaSlopeNorm.toFixed(2)}  accWk=${w.accumWeeks}/${w.weeks}`,
      );
    }
    ...
    for (const d of postBreakout) {
      const dir = d.delta >= 0 ? '+' : '';
      console.log(
        `    ${d.date}: $${d.close.toFixed(2).padStart(7)}  ∂=${dir}${(d.delta / 1000).toFixed(0).padStart(6)}K  (${d.deltaPct.toFixed(1).padStart(6)}%)  vol=${(d.totalVol / 1e6).toFixed(2)}M`,
      );
    }
  } else {
