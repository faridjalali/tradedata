/**
 * VDF Deep Analysis — INSM (Insmed)
 * Full period: 11/27/24 - 8/18/25
 * Breakout: after 5/22/25
 * Suspected slow accumulation
 */
...
function analyzeBreakoutProximity(allDaily, breakoutDate) {
  ...
  if (breakoutIdx < 15) { console.log('  Insufficient pre-breakout data'); return; }
  const preBreakout = allDaily.slice(0, breakoutIdx);
  const n = preBreakout.length;
  const avgVol = mean(preBreakout.map(d => d.totalVol));

  // Day-by-day last 25 days (longer window for slow accum)
  ...
  let streak = 0, streakStart = '', streakDeltas = [];
  for (let i = 0; i < last40.length; i++) {
    if (last40[i].delta < 0) {
      if (streak === 0) { streakStart = last40[i].date; streakDeltas = []; }
      streak++; streakDeltas.push(last40[i].delta);
    } else {
      if (streak >= 2) {
        const fading = streakDeltas.length >= 2 && Math.abs(streakDeltas[streakDeltas.length-1]) < Math.abs(streakDeltas[0]);
        const intensifying = streakDeltas.length >= 2 && Math.abs(streakDeltas[streakDeltas.length-1]) > Math.abs(streakDeltas[0]);
        const label = fading ? ' [FADING]' : intensifying ? ' [INTENSIFYING]' : '';
        console.log(`    ${streakStart}→${last40[i-1].date}: ${streak} red days, deltas: ${streakDeltas.map(d => (d/1000).toFixed(0)+'K').join(', ')}${label}`);
      }
      streak = 0;
    }
  }
  if (streak >= 2) {
    ...
    const intensifying = streakDeltas.length >= 2 && Math.abs(streakDeltas[streakDeltas.length-1]) > Math.abs(streakDeltas[0]);
    const label = fading ? ' [FADING]' : intensifying ? ' [INTENSIFYING]' : '';
    console.log(`    ${streakStart}→${last40[last40.length-1].date}: ${streak} red days, deltas: ${streakDeltas.map(d => (d/1000).toFixed(0)+'K').join(', ')}${label}`);
  }
  ...
  for (const d of last30) {
    if (d.totalVol > avgVol * 1.5) {
      const dir = d.delta >= 0 ? '+' : '';
      console.log(`    ${d.date}: vol=${(d.totalVol/1e6).toFixed(2)}M (${(d.totalVol/avgVol).toFixed(1)}x avg)  ∂=${dir}${(d.delta/1000).toFixed(0)}K  price=$${d.close.toFixed(2)}`);
    }
  }
  ...
  let runCumDelta = 0;
  const cumDeltas = [];
  for (const d of preBreakout) { runCumDelta += d.delta; cumDeltas.push(runCumDelta); }
  for (let i = Math.max(5, n - 35); i < n; i++) {
    const delta5d = cumDeltas[i] - cumDeltas[i - 5];
    const daysLeft = n - i;
    const bar = delta5d > 0 ? '+'.repeat(Math.min(25, Math.round(Math.abs(delta5d)/1000/20))) : '-'.repeat(Math.min(25, Math.round(Math.abs(delta5d)/1000/20)));
    console.log(`    [${String(daysLeft).padStart(2)}d] ${preBreakout[i].date}: 5d∂=${(delta5d/1000).toFixed(0).padStart(7)}K  |${bar}|`);
  }
  ...
  for (let i = 0; i < last30.length; i++) {
    if (last30[i].delta > 0) { if (streak === 0) streakStart = last30[i].date; streak++; }
    else { if (streak >= 2) console.log(`    ${streakStart}→${last30[i-1].date}: ${streak} consecutive green days`); streak = 0; }
  }
  if (streak >= 2) console.log(`    ${streakStart}→${last30[last30.length-1].date}: ${streak} consecutive green days`);

  // IMNM-style anomaly detection: single day with delta > 4x rolling 20d avg
  ...
  for (let i = 20; i < n; i++) {
    ...
    if (avg20Delta > 0 && Math.abs(preBreakout[i].delta) > 4 * avg20Delta) {
      ...
      console.log(`    [${String(daysLeft).padStart(2)}d] ${d.date}: ∂=${dir}${(d.delta/1000).toFixed(0)}K (${mult}x avg)  vol=${(d.totalVol/1e6).toFixed(2)}M  price=$${d.close.toFixed(2)}  ${d.delta > 0 && d.close < (preBreakout[i-1]?.close || d.close) ? '★ABSORB' : ''}`);
    }
  }
}
...
function analyzeRedDeltaAndVolatility(allDaily, breakoutDate) {
  ...
  // Quarters for long period
  const qSize = Math.floor(n / 4);
  const quarters = [preBreakout.slice(0, qSize), preBreakout.slice(qSize, 2*qSize), preBreakout.slice(2*qSize, 3*qSize), preBreakout.slice(3*qSize)];
  const labels = ['Q1', 'Q2', 'Q3', 'Q4'];

  console.log(`\n  By quarters (~${qSize}d each):`);
  console.log(`  ${''.padEnd(5)} ${'RedDays'.padStart(7)} ${'AvgRedMag'.padStart(10)} ${'RedNorm'.padStart(8)} ${'DeltaStd'.padStart(9)} ${'PriceRng'.padStart(9)} ${'NetDelta%'.padStart(10)} ${'Absorption'.padStart(10)}`);
  ...
  for (let t = 0; t < 4; t++) {
    ...
    const netDeltaPct = th.reduce((s,d) => s+d.totalVol,0) > 0 ? th.reduce((s,d) => s+d.delta,0) / th.reduce((s,d) => s+d.totalVol,0) * 100 : 0;
    let absrp = 0;
    for (let i = 1; i < th.length; i++) { if (th[i].close < th[i-1].close && th[i].delta > 0) absrp++; }
    const absrpPct = th.length > 1 ? absrp / (th.length-1) * 100 : 0;
    console.log(`  ${labels[t].padEnd(5)} ${(redD.length+'/'+th.length).padStart(7)} ${((mean(redMags)/1000).toFixed(0)+'K').padStart(10)} ${(redNorm.toFixed(1)+'%').padStart(8)} ${deltaStd.toFixed(1).padStart(9)} ${priceRng.toFixed(2).padStart(9)} ${(netDeltaPct.toFixed(1)+'%').padStart(10)} ${(absrpPct.toFixed(0)+'%').padStart(10)}`);
  }
...
(async () => {
  console.log('=== VDF Deep Analysis — INSM ===\n');

  console.log('Fetching full period (11/27/24 → 8/18/25)...');
  const allBars = await fetch1mChunked('INSM', '2024-11-27', '2025-08-18');
  console.log('Fetching pre-context (10/27/24 → 11/27/24)...');
  const preBars = await fetch1mChunked('INSM', '2024-10-27', '2024-11-27');
  ...
  console.log('='.repeat(80));
  console.log('  INSM — FULL PERIOD OVERVIEW (11/27/24 → 8/18/25)');
  console.log('='.repeat(80));

  // Weekly summary only (daily would be too long for ~180 days)
  console.log('\n  Weekly summary:');
  ...
  // Rolling 5-day delta (show every other day to manage output size)
  console.log(`\n  Rolling 5-day net delta % (every 2nd day for brevity):`);
  ...
  // Subwindow scanning
  console.log(`\n${'='.repeat(80)}`);
  console.log('  INSM — SUBWINDOW SCANNING');
  console.log('='.repeat(80));
  ...
  if (zones.length > 0) {
    console.log(`\n${'='.repeat(80)}`);
    console.log(`  INSM — ${zones.length} ACCUMULATION ZONE(S) DETECTED`);
    console.log('='.repeat(80));
  ...
  // RED DELTA & VOL
  analyzeRedDeltaAndVolatility(allDaily, '2025-05-22');

  // BREAKOUT PROXIMITY
  analyzeBreakoutProximity(allDaily, '2025-05-22');

  // Breakout and post-breakout
  const breakoutIdx = allDaily.findIndex(d => d.date >= '2025-05-22');
  if (breakoutIdx > 0 && breakoutIdx < allDaily.length) {
    console.log(`\n${'='.repeat(80)}`);
    console.log('  INSM — BREAKOUT & POST-BREAKOUT');
    console.log('='.repeat(80));
