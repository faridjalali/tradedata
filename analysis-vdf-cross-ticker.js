...
function computePreBreakoutMetrics(allDaily, breakoutDate) {
  ...
  const priceRangeChange =
    firstAvgPriceRange > 0 ? ((secondAvgPriceRange - firstAvgPriceRange) / firstAvgPriceRange) * 100 : 0;
  ...
  const deltaVolChange =
    firstDeltaVolatility > 0 ? ((secondDeltaVolatility - firstDeltaVolatility) / firstDeltaVolatility) * 100 : 0;
  ...
  return {
    ...
    priceRangeChange,
    // Delta volatility
    firstDeltaVolatility,
    secondDeltaVolatility,
    deltaVolChange,
    // Volume change
    volChangeFirstLast,
  };
...
(async () => {
  ...
  for (const t of TICKERS) {
    ...
    try {
      ...
      console.log(
        `  ✓ ${t.symbol}: ${zones.length} zones, best=${bestZone ? bestZone.score.toFixed(4) : 'N/A'}, breakout=${t.breakout}`,
      );
      console.log(
        `    RED contraction: ${metrics.redSlopeDir} (slope=${metrics.redSlopeNorm.toFixed(2)}%, R²=${metrics.redR2.toFixed(3)})`,
      );
      console.log(
        `    RED norm 1st→2nd half: ${metrics.firstRedNorm.toFixed(1)}%→${metrics.secondRedNorm.toFixed(1)}% (${metrics.redNormChange > 0 ? '+' : ''}${metrics.redNormChange.toFixed(0)}%)`,
      );
      console.log(
        `    Delta vol 1st→2nd half: ${metrics.firstDeltaVolatility.toFixed(1)}→${metrics.secondDeltaVolatility.toFixed(1)} (${metrics.deltaVolChange > 0 ? '+' : ''}${metrics.deltaVolChange.toFixed(0)}%)`,
      );
    } catch (err) {
  ...
  console.log(
    `  ${'Ticker'.padEnd(7)} ${'Score'.padStart(6)} ${'Zones'.padStart(5)} ${'WinSize'.padStart(7)} ${'Best Window'.padEnd(25)} ${'Type'.padEnd(20)}`,
  );
  console.log(`  ${'─'.repeat(90)}`);
  for (const r of results) {
    console.log(
      `  ${r.symbol.padEnd(7)} ${r.bestScore.toFixed(4).padStart(6)} ${r.zoneCount.toString().padStart(5)} ${(r.bestWinSize + 'd').padStart(7)} ${r.bestZone.padEnd(25)} ${r.type.padEnd(20)}`,
    );
  }
  ...
  console.log(`  ${'─'.repeat(100)}`);
  console.log(
    `  ${'Ticker'.padEnd(7)} ${'1stNorm'.padStart(7)} ${'2ndNorm'.padStart(7)} ${'Change'.padStart(7)} ${'Direction'.padEnd(10)} ${'Slope%'.padStart(7)} ${'R²'.padStart(6)} ${'Last10vs'.padStart(8)} ${'E→M→L (thirds)'.padEnd(22)}`,
  );
  console.log(`  ${'─'.repeat(100)}`);
  for (const r of results) {
    const thirdsStr = r.thirdRedNorms.map((v) => v.toFixed(1)).join('→');
    console.log(
      `  ${r.symbol.padEnd(7)} ${(r.firstRedNorm.toFixed(1) + '%').padStart(7)} ${(r.secondRedNorm.toFixed(1) + '%').padStart(7)} ${(r.redNormChange > 0 ? '+' : '') + r.redNormChange.toFixed(0) + '%'.padStart(0)} ${r.redSlopeDir.padEnd(10)} ${r.redSlopeNorm.toFixed(2).padStart(7)} ${r.redR2.toFixed(3).padStart(6)} ${(r.last10VsOverall > 0 ? '+' : '') + r.last10VsOverall.toFixed(0) + '%'} ${thirdsStr}`,
    );
  }
  ...
  console.log(
    `  ${'Ticker'.padEnd(7)} ${'1stStd'.padStart(7)} ${'2ndStd'.padStart(7)} ${'Change'.padStart(7)} ${'Delta Rng: E→M→L'.padEnd(25)} ${'PriceRange'.padStart(10)}`,
  );
  console.log(`  ${'─'.repeat(80)}`);
  for (const r of results) {
    const drStr = r.thirdDeltaRanges.map((v) => v.toFixed(1)).join('→');
    console.log(
      `  ${r.symbol.padEnd(7)} ${r.firstDeltaVolatility.toFixed(1).padStart(7)} ${r.secondDeltaVolatility.toFixed(1).padStart(7)} ${((r.deltaVolChange > 0 ? '+' : '') + r.deltaVolChange.toFixed(0) + '%').padStart(7)} ${drStr.padEnd(25)} ${((r.priceRangeChange > 0 ? '+' : '') + r.priceRangeChange.toFixed(0) + '%').padStart(10)}`,
    );
  }
  ...
  console.log(`  ${'─'.repeat(80)}`);
  console.log(
    `  ${'Ticker'.padEnd(7)} ${'1stAbsrp'.padStart(8)} ${'2ndAbsrp'.padStart(8)} ${'1stGreen'.padStart(8)} ${'2ndGreen'.padStart(8)} ${'MaxConsecRed 1→2'.padEnd(20)} ${'Red%'.padStart(5)}`,
  );
  console.log(`  ${'─'.repeat(80)}`);
  for (const r of results) {
    console.log(
      `  ${r.symbol.padEnd(7)} ${(r.firstAbsorption.toFixed(0) + '%').padStart(8)} ${(r.secondAbsorption.toFixed(0) + '%').padStart(8)} ${(r.firstGreenRatio * 100).toFixed(0).padStart(7) + '%'} ${(r.secondGreenRatio * 100).toFixed(0).padStart(7) + '%'} ${(r.firstMaxConsecRed + '→' + r.secondMaxConsecRed).padEnd(20)} ${r.redPct.toFixed(0).padStart(4) + '%'}`,
    );
  }
  ...
  for (const r of results) {
    console.log(
      `  ${r.symbol.padEnd(7)} ${((r.volChangeFirstLast > 0 ? '+' : '') + r.volChangeFirstLast.toFixed(0) + '%').padStart(15)} ${r.firstRedStdNorm.toFixed(1) + '%→' + r.secondRedStdNorm.toFixed(1) + '%'}`,
    );
  }
  ...
  console.log(
    `    Contracting: ${contracting.length}/${results.length} (${contracting.map((r) => r.symbol).join(', ')})`,
  );
  ...
  console.log(
    `    Contracting: ${dVolContracting.length}/${results.length} (${dVolContracting.map((r) => r.symbol).join(', ')})`,
  );
  console.log(
    `    Expanding:   ${dVolExpanding.length}/${results.length} (${dVolExpanding.map((r) => r.symbol).join(', ')})`,
  );
  ...
  console.log(
    `    Contracting: ${pVolContracting.length}/${results.length} (${pVolContracting.map((r) => r.symbol).join(', ')})`,
  );
  ...
  console.log(
    `    Yes: ${greenIncreasing.length}/${results.length} (${greenIncreasing.map((r) => r.symbol).join(', ')})`,
  );
  console.log(
    `    Avg 1st: ${(mean(results.map((r) => r.firstGreenRatio)) * 100).toFixed(0)}%  Avg 2nd: ${(mean(results.map((r) => r.secondGreenRatio)) * 100).toFixed(0)}%`,
  );
  ...
  console.log(
    `    ${corrReg.r2 < 0.1 ? 'WEAK/NO correlation — RED contraction not predictive of score' : corrReg.r2 < 0.3 ? 'MODERATE correlation' : 'STRONG correlation'}`,
  );
  ...
  console.log(
    `    ${corrReg2.r2 < 0.1 ? 'WEAK/NO correlation — Delta vol contraction not predictive of score' : corrReg2.r2 < 0.3 ? 'MODERATE correlation' : 'STRONG correlation'}`,
  );
