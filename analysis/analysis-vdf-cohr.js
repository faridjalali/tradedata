...
function analyzeMonthlyPhases(allDaily) {
  ...
  console.log(
    `  ${'Month'.padEnd(22)} ${'PriceChg'.padStart(9)} ${'NetDelta%'.padStart(10)} ${'CumDelta'.padStart(10)} ${'AbsorbPct'.padStart(10)} ${'AvgVol'.padStart(8)} ${'Signal'.padStart(12)}`,
  );
  ...
  for (let i = 20; i < allDaily.length; i += 20) {
    ...
    console.log(
      `  ${chunk[0].date}→${chunk[chunk.length - 1].date} ${(priceChg >= 0 ? '+' : '') + priceChg.toFixed(1) + '%'} ${(netDeltaPct >= 0 ? '+' : '') + netDeltaPct.toFixed(1) + '%'}  ${(netDelta / 1000 >= 0 ? '+' : '') + (netDelta / 1000).toFixed(0) + 'K'}  ${absorbPct.toFixed(0) + '%'}  ${(avgVol / 1e6).toFixed(1) + 'M'}  ${signal}`,
    );
  }
}
...
function findDistributionWindows(allDaily) {
  ...
  if (clusters.length > 0) {
    ...
    for (const c of clusters) {
      ...
      console.log(
        `    ${c.startDate}→${c.endDate} (${span}d): price ${fullPriceChg >= 0 ? '+' : ''}${fullPriceChg.toFixed(1)}%, delta ${fullDeltaPct >= 0 ? '+' : ''}${fullDeltaPct.toFixed(1)}% (${(fullDelta / 1000).toFixed(0)}K) — ${c.count} overlapping windows`,
      );
  ...
  if (accumClusters.length > 0) {
    for (const c of accumClusters) {
      ...
      const fullDeltaPct = fullVol > 0 ? (fullDelta / fullVol) * 100 : 0;
      console.log(
        `    ${c.startDate}→${c.endDate} (${span}d): price ${fullPriceChg.toFixed(1)}%, delta +${fullDeltaPct.toFixed(1)}% — ★ ENTRY SIGNAL`,
      );
    }
  } else {
...
function detectBreakouts(allDaily) {
    ...
    if (priceChange > 8 && volRatio > 1.2) {
      ...
      if (!tooClose) {
        breakouts.push({
          ...
          priceChange,
          volRatio,
          priceAtBreakout: window[4].close,
          priceAtStart: window[0].close,
        });
      }
    }
  }
  ...
  for (const b of breakouts) {
    ...
    const netDeltaPct = totalVol > 0 ? (netDelta / totalVol) * 100 : 0;
    console.log(
      `    Breakout delta: ${netDeltaPct >= 0 ? '+' : ''}${netDeltaPct.toFixed(1)}% (${netDelta >= 0 ? '+' : ''}${(netDelta / 1000).toFixed(0)}K) — ${netDeltaPct > 2 ? '✓ confirmed by delta' : netDeltaPct < -2 ? '⚠ distribution into rally' : '— neutral delta'}`,
    );
    ...
    if (postEnd - postStart >= 10) {
      ...
      console.log(
        `    Post-breakout (4wk): ${posWeeks}/${postWeeks.length} positive delta weeks → ${posWeeks >= 3 ? 'DURABLE' : posWeeks <= 1 ? 'FRAGILE' : 'MIXED'}`,
      );
    }
  }
...
function analyzeProximityBeforeBreakout(allDaily, breakoutIdx) {
  ...
  for (let i = 0; i < last15.length; i++) {
    ...
    console.log(
      `      [${String(daysLeft).padStart(2)}d] ${d.date}: $${d.close.toFixed(2).padStart(7)} (${priceChg.padStart(6)}%)  ∂=${dir}${(d.delta / 1000).toFixed(0).padStart(6)}K  (${d.deltaPct.toFixed(1).padStart(6)}%)${absorb}`,
    );
  }
  ...
  for (let i = 20; i < n; i++) {
    ...
    if (avg20 > 0 && Math.abs(pre[i].delta) > 4 * avg20) {
      ...
      if (daysLeft <= 30) {
        signals.push(
          `Delta anomaly: ${pre[i].date} (${(Math.abs(pre[i].delta) / avg20).toFixed(1)}x), ${daysLeft}d before`,
        );
        points += 25;
      }
    }
  ...
  for (let i = 0; i < last20.length; i++) {
    if (last20[i].delta > 0) streak++;
      ...
      if (streak >= 4) {
        signals.push(`Green streak: ${streak} days`);
        points += 20;
      }
      streak = 0;
    }
  ...
  if (absCount >= 3) {
    signals.push(`Absorption cluster: ${absCount} in 15 days`);
    points += 15;
  }
...
function buildSwingTimeline(allDaily, zones, breakouts, distClusters) {
  ...
  for (const z of zones) {
    events.push({
      date: z.startDate,
      type: 'ACCUM_START',
      detail: `Zone score ${z.score.toFixed(2)} (${z.winSize}d), net∂=${z.netDeltaPct.toFixed(1)}%, absorb=${z.absorptionPct.toFixed(0)}%`,
      score: z.score,
    });
    events.push({
      date: z.endDate,
      type: 'ACCUM_END',
      detail: `Zone complete — ${z.accumWeeks}/${z.weeks} weeks positive`,
      score: z.score,
    });
  }
  ...
  for (const b of breakouts) {
    events.push({
      date: b.date,
      type: 'BREAKOUT',
      detail: `+${b.priceChange.toFixed(1)}% in 5d, vol=${b.volRatio.toFixed(1)}x`,
      score: 0,
    });
  }
  ...
  for (const d of distClusters) {
    events.push({
      date: d.startDate,
      type: 'DISTRIB_START',
      detail: `Price up but delta negative — institutions selling`,
      score: 0,
    });
    ...
    const action =
      e.type === 'ACCUM_START'
        ...
        : e.type === 'ACCUM_END'
          ? '→ HOLD / ADD ON DIPS'
          : e.type === 'BREAKOUT'
            ? '→ BREAKOUT — ride momentum'
            : e.type === 'DISTRIB_START'
              ? '→ ⚠ START TAKING PROFITS'
              : e.type === 'DISTRIB_END'
                ? '→ Distribution phase over'
                : '';
...
(async () => {
  ...
  for (const w of allWeeks) {
    ...
    cumDelta += w.delta;
    console.log(
      `    ${w.weekStart}: ${priceDir} $${w.open.toFixed(2).padStart(7)}→$${w.close.toFixed(2).padStart(7)}  ∂=${dir}${w.deltaPct.toFixed(2).padStart(7)}%  (${dir}${(w.delta / 1000).toFixed(0).padStart(7)}K)  cum∂=${(cumDelta / 1000 >= 0 ? '+' : '') + (cumDelta / 1000).toFixed(0).padStart(7)}K  vol=${(w.avgVol / 1e6).toFixed(1)}M  [${w.nDays}d]`,
    );
  }
  ...
  if (zones.length > 0) {
    ...
    for (let zi = 0; zi < zones.length; zi++) {
      ...
      for (const w of z.weeksData) {
        ...
        const priceDir = w.close >= w.open ? '▲' : '▼';
        console.log(
          `      ${w.weekStart}: ${priceDir} $${w.close.toFixed(2).padStart(7)}  ${dir}${(w.effectiveDelta / 1000).toFixed(0).padStart(6)}K  (${dir}${w.deltaPct.toFixed(2).padStart(7)}%)  [${w.nDays}d]`,
        );
      }
  ...
  for (let i = 20; i < allDaily.length; i++) {
    ...
    if (avg20 > 0 && Math.abs(allDaily[i].delta) > 4 * avg20) {
      ...
      console.log(
        `    ${d.date}: ∂=${dir}${(d.delta / 1000).toFixed(0)}K (${mult}x avg)  vol=${(d.totalVol / 1e6).toFixed(2)}M  price=$${d.close.toFixed(2)}${absorb}`,
      );
    }
  }
  ...
  for (let i = 0; i < allDaily.length; i++) {
    if (allDaily[i].delta < 0) {
      if (exStreak === 0) {
        exStart = allDaily[i].date;
        exDeltas = [];
      }
      exStreak++;
      exDeltas.push(allDaily[i].delta);
    } else {
      if (exStreak >= 3) {
  ...
  for (let i = 0; i < allDaily.length; i++) {
    if (allDaily[i].delta > 0) {
      if (gStreak === 0) gStart = allDaily[i].date;
      gStreak++;
    } else {
  ...
  if (gStreak >= 4)
    console.log(`    ${gStart}→${allDaily[allDaily.length - 1].date}: ${gStreak} consecutive green days`);
