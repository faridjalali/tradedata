/**
 * VDF Deep Analysis — SATS (EchoStar / Hughes Network)
 * Full period: 3/3/25 - 7/31/25
 * Breakout: 6/16/25
 * Question: Fast or slow accumulation?
 * Also: RED delta volatility contraction analysis
 */
...
/**
 * RED DELTA VOLATILITY CONTRACTION ANALYSIS
 * Measures whether sell-side (negative) delta magnitudes diminish approaching breakout.
 * Concept: as institutional accumulation completes, sellers dry up — not just less volume,
 * but each sell day's delta becomes smaller in magnitude (less conviction selling).
 */
function analyzeRedDeltaContraction(allDaily, breakoutDate) {
  console.log(`\n${'='.repeat(80)}`);
  console.log('  RED DELTA VOLATILITY CONTRACTION ANALYSIS');
  ...
  console.log(`\n  Rolling 5-day RED delta analysis:`);
  console.log(`    (window avg of |negative delta| on red days, and count of red days in window)`);
  ...
  const halfIdx = Math.floor(preBreakoutDaily.length / 2);
  ...
  const thirds = [
    preBreakoutDaily.slice(0, thirdSize),
    preBreakoutDaily.slice(thirdSize, 2 * thirdSize),
    preBreakoutDaily.slice(2 * thirdSize),
  ];
  console.log(`\n  RED delta by thirds (early → mid → late):`);
  console.log(`  ──────────────────────────────────────────`);
  const thirdLabels = ['Early', 'Mid', 'Late'];
  const thirdResults = [];
  for (let t = 0; t < 3; t++) {
    const th = thirds[t];
...
(async () => {
  console.log('=== VDF Deep Analysis — SATS ===\n');

  console.log('Fetching full period (3/3/25 → 7/31/25)...');
  const allBars = await fetch1mChunked('SATS', '2025-03-03', '2025-07-31');
  console.log('Fetching pre-context (2/1/25 → 3/3/25)...');
  const preBars = await fetch1mChunked('SATS', '2025-02-01', '2025-03-03');
  ...
  console.log('='.repeat(80));
  console.log('  SATS — FULL PERIOD OVERVIEW (3/3/25 → 7/31/25)');
  console.log('='.repeat(80));
  ...
  // Subwindow scanning
  console.log(`\n${'='.repeat(80)}`);
  console.log('  SATS — SUBWINDOW SCANNING');
  console.log('='.repeat(80));
  ...
  if (zones.length > 0) {
    console.log(`\n${'='.repeat(80)}`);
    console.log(`  SATS — ${zones.length} ACCUMULATION ZONE(S) DETECTED`);
    console.log('='.repeat(80));
  ...
  // RED DELTA VOLATILITY CONTRACTION ANALYSIS
  // Analyze just the pre-breakout portion (before 6/16/25)
  analyzeRedDeltaContraction(allDaily, '2025-06-16');

  // End-of-period analysis (last 10 days before breakout)
  console.log(`\n${'='.repeat(80)}`);
