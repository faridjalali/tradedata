...
(async () => {
  ...
  if (directResult) {
    ...
    console.log(
      `  Price: $${closes[0]?.toFixed(2)} → $${closes[closes.length - 1]?.toFixed(2)} (${(((closes[closes.length - 1] - closes[0]) / closes[0]) * 100).toFixed(1)}%)`,
    );
    console.log(
      `  Score: ${directResult.score.toFixed(4)} ${directResult.detected ? '✅ DETECTED' : '❌ NOT DETECTED'}`,
    );
    ...
    if (directResult.weeksData) {
      ...
      for (const w of directResult.weeksData) {
        ...
        console.log(
          `    ${w.weekStart}: ${dir}${w.deltaPct.toFixed(2)}%  (${dir}${(w.delta / 1000).toFixed(0)}K)  rng=${w.avgRange.toFixed(2)}%  vol=${(w.avgVol / 1e6).toFixed(2)}M  [${w.nDays}d]`,
        );
      }
    }
  } else {
