...
for (const ticker of tickers) {
  ...
  for (const z of d.jsAlgorithm.zones) {
    console.log(
      `  Z${z.rank}: ${z.startDate} → ${z.endDate} | ${z.windowDays}d | score=${z.score} | netΔ=${z.netDeltaPct}% | price=${z.overallPriceChange}% | absorption=${z.absorptionPct}% | concordant=${z.concordantFrac} | penalty=${z.concordancePenalty} | durMult=${z.durationMultiplier}`,
    );
    if (z.components) {
      console.log(
        `    Components: s1=${z.components.s1_netDelta} s2=${z.components.s2_deltaSlope} s3=${z.components.s3_deltaShift} s4=${z.components.s4_accumRatio} s5=${z.components.s5_buyVsSell} s6=${z.components.s6_absorption} s7=${z.components.s7_volDecline} s8=${z.components.s8_divergence}`,
      );
    }
  }
  ...
  for (const dist of d.jsAlgorithm.distribution) {
    console.log(
      `  D: ${dist.startDate} → ${dist.endDate} | ${dist.spanDays}d | price=${dist.priceChangePct}% | netΔ=${dist.netDeltaPct}%`,
    );
  }
  ...
  for (const w of d.weeklySummary) {
    ...
    console.log(
      `${w.weekStart} | ${w.days}    | ${w.priceChgPct >= 0 ? '+' : ''}${w.priceChgPct.toFixed(1).padStart(5)}% | ${(w.delta / 1000).toFixed(0).padStart(7)}K | ${dir}${w.deltaPct.toFixed(2).padStart(5)}% | ${(w.cumDelta / 1000).toFixed(0).padStart(7)}K | ${w.absorptionDays}   | ${(w.avgVol / 1e6).toFixed(1).padStart(5)}M | $${w.closeEnd}`,
    );
  }
          ...
          : p.phase === 'CONCORDANT_DECLINE'
            ...
            : p.phase === 'CONFIRMED_RALLY'
              ...
              : p.phase === 'ABSORBING'
                ...
                : '    ';
    console.log(
      `${marker}${p.startDate} → ${p.endDate} | ${p.phase.padEnd(24)} | price=${p.priceChgPct >= 0 ? '+' : ''}${p.priceChgPct}% | netΔ=${p.netDeltaPct >= 0 ? '+' : ''}${p.netDeltaPct}% | abs=${p.absorptionDays}`,
    );
  }
  ...
  if (d.deltaAnomalies.length > 0) {
    ...
    for (const a of d.deltaAnomalies) {
      console.log(
        `  ${a.date}: ${(a.delta / 1000).toFixed(0)}K (${a.ratio}x avg) | price=${a.priceChgPct}% | $${a.close} | ${a.type}`,
      );
    }
  }
  ...
  if (d.streaks.length > 0) {
    ...
    for (const s of d.streaks) {
      console.log(
        `  ${s.type.toUpperCase()} ${s.days}d: ${s.startDate} → ${s.endDate} | totalΔ=${(s.totalDelta / 1000).toFixed(0)}K${s.intensifying ? ' (INTENSIFYING)' : ''}`,
      );
    }
  }
}
