...
function scoreWithGateAnalysis(consolBars1m, preBars1m, label) {
  ...
  return {
    ...
    // Current algorithm
    currentScore,
    currentDetected: currentScore >= 0.3,
    // Gate 2: Delta slope
    deltaSlopeNorm,
    dailySlopeNorm,
    ...
    cappedDeltaShift,
    cappedScore,
    cappedDetected: cappedScore >= 0.3,
...
(async () => {
  ...
  const cases = [
    {
      symbol: 'RKLB',
      label: 'RKLB (positive — accumulation 2/26-4/7/2025)',
    ...
    {
      symbol: 'IREN',
      label: 'IREN (positive — accumulation 3/13-4/21/2025)',
    ...
    {
      symbol: 'TRON',
      label: 'TRON (FALSE POSITIVE — should be rejected)',
  ...
  for (const r of results) {
    console.log(`\n--- ${r.label} ---`);
    console.log(
      `  Expected: ${r.expected ? 'DETECTED' : 'REJECTED'}   |   Current algorithm: ${r.currentDetected ? 'DETECTED' : 'REJECTED'} (score ${r.currentScore.toFixed(4)})`,
    );
    console.log(
      `  Price change: ${r.overallPriceChange.toFixed(1)}%   Weeks: ${r.weeks}   Accum weeks: ${r.accumWeeks}/${r.weeks} (${(r.accumWeekRatio * 100).toFixed(0)}%)`,
    );
    console.log();
    ...
    for (const thresh of [-2.0, -1.5, -1.0, -0.5, 0]) {
      ...
      console.log(
        `    Gate at slope < ${thresh.toFixed(1)}: ${wouldReject ? 'REJECT ✗' : 'PASS ✓'}${wouldReject !== !r.expected ? ' ← WRONG' : ''}`,
      );
    }
    console.log();
    ...
    console.log(
      `    Original score:        ${r.currentScore.toFixed(4)} → ${r.currentDetected ? 'DETECTED' : 'REJECTED'}`,
    );
    console.log(
      `    Capped score:          ${r.cappedScore.toFixed(4)} → ${r.cappedDetected ? 'DETECTED' : 'REJECTED'}${r.cappedDetected !== r.expected ? ' ← WRONG' : ''}`,
    );
    console.log();
    ...
    for (const thresh of [0.5, 0.3, 0.0, -0.1, -0.2]) {
      ...
      console.log(
        `    Gate at corr > ${thresh >= 0 ? '+' : ''}${thresh.toFixed(1)} (original):  ${wouldReject ? 'REJECT ✗' : 'PASS ✓'}${wouldReject !== !r.expected ? ' ← WRONG' : ''}`,
      );
      console.log(
        `    Gate at corr > ${thresh >= 0 ? '+' : ''}${thresh.toFixed(1)} (capped):    ${cappedWouldReject ? 'REJECT ✗' : 'PASS ✓'}${cappedWouldReject !== r.expected ? ' ← WRONG' : ''}`,
      );
    }
    console.log();
  ...
  const header =
    ...
    'Expected'.padEnd(10) +
    'Current'.padEnd(10) +
    'G2(slope<-1)'.padEnd(14) +
    'G3(capped)'.padEnd(14) +
    'G4(corr>0.3)'.padEnd(14) +
    'G3+G4'.padEnd(10);
  console.log(header);
  ...
  for (const r of results) {
    ...
    const g3_det = r.cappedScore >= 0.3 ? 'DETECT' : 'REJECT';
    ...
    const g3g4 = r.cappedScore >= 0.3 && r.cappedPriceDeltaCorr <= 0.3 ? 'DETECT' : 'REJECT';
  ...
  for (const combo of combos) {
    const outcomes = results.map((r) => {
      let detected = combo.useCap ? r.cappedScore >= 0.3 : r.currentScore >= 0.3;
