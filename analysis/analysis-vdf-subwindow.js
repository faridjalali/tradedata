...
function scoreSubwindow(dailySlice, preBars1m, allDailyAvgVol) {
  ...
  if (overallPriceChange > 10)
    return { score: 0, detected: false, reason: 'price_rising', weeks: weeks.length, overallPriceChange };
  if (overallPriceChange < -45)
    return { score: 0, detected: false, reason: 'crash', weeks: weeks.length, overallPriceChange };
...
async function analyzeTickerSubwindows(symbol, consolFrom, consolTo, preFrom, preTo) {
  ...
  console.log('='.repeat(70));
  console.log(
    `  Period: ${allDaily[0].date} → ${allDaily[allDaily.length - 1].date} (${allDaily.length} trading days)`,
  );
  console.log();
  ...
  for (const d of allDaily) {
    ...
    console.log(
      `    ${d.date}: $${d.close.toFixed(2).padStart(7)}  delta=${dir}${(d.delta / 1000).toFixed(0).padStart(6)}K  (${pctOfVol.padStart(6)}%)  vol=${(d.totalVol / 1e6).toFixed(2)}M`,
    );
  }
  console.log();
  ...
  for (const w of allWeeks) {
    const dir = w.delta >= 0 ? '+' : '';
    console.log(
      `    ${w.weekStart}: ${dir}${w.deltaPct.toFixed(2).padStart(7)}%  (${dir}${(w.delta / 1000).toFixed(0).padStart(7)}K)  vol=${(w.totalVol / 1e6).toFixed(1)}M  [${w.nDays}d]`,
    );
  }
  console.log();
  ...
  console.log(
    `  Full window score: ${fullResult ? fullResult.score.toFixed(4) : 'N/A'} ${fullResult?.detected ? '✅ DETECTED' : '❌ NOT DETECTED'} ${fullResult?.reason ? `(${fullResult.reason})` : ''}`,
  );
  if (fullResult && fullResult.netDeltaPct !== undefined) {
    console.log(
      `    Net delta: ${fullResult.netDeltaPct.toFixed(2)}%  Price: ${fullResult.overallPriceChange.toFixed(1)}%  Corr: ${fullResult.priceDeltaCorr?.toFixed(2) || 'N/A'}  Slope: ${fullResult.deltaSlopeNorm?.toFixed(2) || 'N/A'}`,
    );
  }
  console.log();
  ...
  for (const winSize of windowSizes) {
    ...
    if (top.length === 0) {
      ...
      for (const w of top) {
        ...
        const gated =
          w.reason === 'concordant_selling'
            ? ' [GATED: concordant]'
            : w.reason === 'crash'
              ? ' [GATED: crash]'
              : w.reason === 'price_rising'
                ? ' [GATED: rising]'
                : '';
        console.log(
          `    ${det} ${w.dateRange}  score=${w.score.toFixed(4)}  net∂=${w.netDeltaPct?.toFixed(2) || 'N/A'}%  price=${w.overallPriceChange.toFixed(1)}%  corr=${w.priceDeltaCorr?.toFixed(2) || 'N/A'}  slope=${w.deltaSlopeNorm?.toFixed(2) || 'N/A'}  wks=${w.weeks}${gated}`,
        );
      }
    }
  ...
  if (allResults.length > 0) {
    ...
    console.log(
      `    s1 Net Delta (22%):       ${best.components.s1.toFixed(4)}  (netDeltaPct=${best.netDeltaPct.toFixed(2)}%)`,
    );
    console.log(
      `    s2 Delta Slope (18%):     ${best.components.s2.toFixed(4)}  (slope=${best.deltaSlopeNorm.toFixed(2)})`,
    );
    console.log(
      `    s3 Delta Shift (15%):     ${best.components.s3.toFixed(4)}  (shift=${best.deltaShift.toFixed(2)})`,
    );
    console.log(
      `    s4 Absorption (13%):      ${best.components.s4.toFixed(4)}  (${best.strongAbsorptionPct.toFixed(1)}%)`,
    );
    console.log(`    s5 Buy vs Sell (8%):      ${best.components.s5.toFixed(4)}  (${best.largeBuyVsSell.toFixed(1)})`);
    console.log(
      `    s6 Anti-corr (9%):        ${best.components.s6.toFixed(4)}  (corr=${best.priceDeltaCorr.toFixed(2)})`,
    );
    ...
    console.log(
      `    G2 slope < -1.5:     ${best.deltaSlopeNorm < -1.5 ? 'REJECT' : 'PASS'} (slope=${best.deltaSlopeNorm.toFixed(2)})`,
    );
    console.log(
      `    G4 corr > +0.5:      ${best.priceDeltaCorr > 0.5 ? 'REJECT' : 'PASS'} (corr=${best.priceDeltaCorr.toFixed(2)})`,
    );
    console.log(
      `    G2(-2)+G4(0.5):      ${best.deltaSlopeNorm < -2 || best.priceDeltaCorr > 0.5 ? 'REJECT' : 'PASS'}`,
    );
  } else {
    console.log('='.repeat(70));
    ...
    for (const w of allNonDetected.slice(0, 10)) {
      const gated = w.reason === 'concordant_selling' ? ' [concordant]' : w.reason === 'crash' ? ' [crash]' : '';
      console.log(
        `    ${w.dateRange} (${w.winSize}d)  score=${w.score.toFixed(4)}  net∂=${w.netDeltaPct?.toFixed(2) || '?'}%  price=${w.overallPriceChange.toFixed(1)}%  corr=${w.priceDeltaCorr?.toFixed(2) || '?'}  slope=${w.deltaSlopeNorm?.toFixed(2) || '?'}${gated}`,
      );
    }
    ...
    if (nearMiss.length > 0) {
      for (const w of nearMiss) {
        console.log(
          `    ${w.dateRange} (${w.winSize}d)  net∂=${w.netDeltaPct.toFixed(2)}%  price=${w.overallPriceChange.toFixed(1)}%`,
        );
      }
    } else {
