...
function buildWeeks(daily) {
  ...
  return [...weekMap.entries()].sort((a, b) => a[0].localeCompare(b[0])).map(([weekStart, days]) => {
    ...
    return {
      ...
      nDays: days.length,
      open: opens[0], close: closes[closes.length - 1],
...
function deepAnalysis(daily, preBars1m, label) {
  ...
  return {
    label, nDays: n, nWeeks: weeks.length,
    overallPriceChange, netDeltaPct,
    // Delta
    deltaSlopeNorm, weeklySlopeNorm, priceDeltaCorr,
    deltaShift, accumWeeks, accumWeekRatio: accumWeeks / weeks.length,
    // Volatility
    avgT1Range, avgT2Range, avgT3Range,
    rangeContractionT3vsT1, rangeSlopeR2: rangeSlopeReg.r2, rangeSlopeDir: rangeSlopeReg.slope < 0 ? 'contracting' : 'expanding',
    avgT1Vol, avgT2Vol, avgT3Vol, volContractionT3vsT1,
    bbContraction, bbFirst5, bbLast5,
    // Pre-breakout
    last5DeltaPct, last10DeltaPct,
    last5AvgRange, last10AvgRange, first5AvgRange, first10AvgRange,
    last5AvgVol, last10AvgVol, first5AvgVol, first10AvgVol,
    volDryup, rangeCompression,
    // Outliers
    outliers2sigma: outliers2sigma.map(d => ({ date: d.date, delta: d.delta, deltaPct: d.deltaPct, vol: d.totalVol })),
    outliers3sigma: outliers3sigma.map(d => ({ date: d.date, delta: d.delta, deltaPct: d.deltaPct, vol: d.totalVol })),
    volOutliers: volOutliers.map(d => ({ date: d.date, vol: d.totalVol, delta: d.delta })),
    // Absorption
    absorptionDays, strongAbsorptionDays, distributionDays,
    absorptionPct: n > 1 ? (absorptionDays / (n - 1)) * 100 : 0,
    distributionPct: n > 1 ? (distributionDays / (n - 1)) * 100 : 0,
    // Price structure
    pricePositionInRange, daysNearLow, daysNearHigh,
    periodHigh, periodLow, lastClose,
    // Best subwindow
    bestSubwindow,
    // Raw data for printing
    daily, weeks, preDaily,
  };
}
...
function scoreSubwindow(dailySlice, preDaily) {
  ...
  return {
    ...
    deltaSlopeNorm, priceDeltaCorr, accumWeekRatio,
    weeks: weeks.length, accumWeeks,
...
function printDeepAnalysis(r) {
  ...
  console.log(`  Price: $${r.daily[0].close.toFixed(2)} → $${r.lastClose.toFixed(2)} (${r.overallPriceChange.toFixed(1)}%)`);
  console.log(`  Price range: $${r.periodLow.toFixed(2)} – $${r.periodHigh.toFixed(2)}, last close at ${r.pricePositionInRange.toFixed(0)}% of range`);
  console.log(`  Days near low (bottom 20%): ${r.daysNearLow}/${r.nDays}   Days near high (top 20%): ${r.daysNearHigh}/${r.nDays}`);
  ...
  console.log(`    Contraction:  ${r.rangeContractionT3vsT1.toFixed(1)}% ${r.rangeContractionT3vsT1 < -15 ? '✅ CONTRACTING' : r.rangeContractionT3vsT1 > 15 ? '⬆ EXPANDING' : '~ FLAT'}`);
  ...
  console.log(`    Change:       ${r.volContractionT3vsT1.toFixed(1)}% ${r.volContractionT3vsT1 < -15 ? '✅ DECLINING' : '~ FLAT/RISING'}`);
  ...
  console.log(`    Change: ${r.bbContraction.toFixed(1)}% ${r.bbContraction < -15 ? '✅ NARROWING' : r.bbContraction > 15 ? '⬆ WIDENING' : '~ FLAT'}`);
  ...
  console.log(`  Range:    first5=${r.first5AvgRange.toFixed(3)}%  last5=${r.last5AvgRange.toFixed(3)}%  (${r.rangeCompression.toFixed(1)}% vs avg)`);
  console.log(`  Volume:   first5=${(r.first5AvgVol/1e6).toFixed(2)}M  last5=${(r.last5AvgVol/1e6).toFixed(2)}M  (${r.volDryup.toFixed(1)}% vs avg)`);

  console.log(`\n  ── OUTLIERS ──`);
  if (r.outliers3sigma.length > 0) {
    console.log(`  3σ delta outliers:`);
    for (const o of r.outliers3sigma) {
      console.log(`    ${o.date}: delta=${(o.delta/1000).toFixed(0)}K (${o.deltaPct.toFixed(1)}%) vol=${(o.vol/1e6).toFixed(2)}M`);
    }
  } else {
  ...
  for (const w of r.weeks) {
    ...
    console.log(`  ${w.weekStart}: ${priceDir} $${w.open.toFixed(2)}→$${w.close.toFixed(2)}  ∂=${dir}${w.deltaPct.toFixed(2)}%  (${dir}${(w.delta/1000).toFixed(0)}K)  range=${w.avgRange.toFixed(3)}%  vol=${(w.avgVol/1e6).toFixed(2)}M  [${w.nDays}d]`);
  }
  ...
  for (let i = 0; i < last10.length; i++) {
    ...
    console.log(`  ${d.date}: $${d.close.toFixed(2)} (${priceChg}%)  ∂=${dir}${(d.delta/1000).toFixed(0)}K (${d.deltaPct.toFixed(1)}%)  range=${d.rangePct.toFixed(3)}%  vol=${(d.totalVol/1e6).toFixed(2)}M${absorb}`);
  }
  ...
  if (r.bestSubwindow) {
    ...
    console.log(`  Price: ${b.overallPriceChange.toFixed(1)}%  Net∂: ${b.netDeltaPct.toFixed(2)}%  Corr: ${b.priceDeltaCorr.toFixed(2)}  Slope: ${b.deltaSlopeNorm.toFixed(2)}`);
    console.log(`  Accum weeks: ${b.accumWeeks}/${b.weeks}`);
    console.log(`  Components: s1=${b.components.s1.toFixed(2)} s2=${b.components.s2.toFixed(2)} s3=${b.components.s3.toFixed(2)} s4=${b.components.s4.toFixed(2)} s5=${b.components.s5.toFixed(2)} s6=${b.components.s6.toFixed(2)} s7=${b.components.s7.toFixed(2)} s8=${b.components.s8.toFixed(2)}`);
  }
}
...
(async () => {
  const cases = [
    {
      symbol: 'RKLB', label: 'RKLB — Accumulation Feb-Apr 2025 (breakout ~Apr 7)',
      consolFrom: '2025-02-26', consolTo: '2025-04-07',
      preFrom: '2025-01-27', preTo: '2025-02-26',
    },
    {
      symbol: 'IREN', label: 'IREN — Accumulation Mar-Apr 2025 (breakout ~Apr 21)',
      consolFrom: '2025-03-13', consolTo: '2025-04-21',
      preFrom: '2025-02-11', preTo: '2025-03-13',
    },
    {
      symbol: 'HUT', label: 'HUT — Accumulation Feb-Apr 2025 (breakout ~Apr 21)',
      consolFrom: '2025-02-24', consolTo: '2025-04-21',
      preFrom: '2025-01-25', preTo: '2025-02-24',
    },
    {
      symbol: 'AFRM', label: 'AFRM — Accumulation Feb-Aug 2024 (breakout ~Aug 6)',
      consolFrom: '2024-02-09', consolTo: '2024-08-06',
      preFrom: '2024-01-10', preTo: '2024-02-09',
    },
    {
      symbol: 'ALAB', label: 'ALAB — Accumulation Feb-Apr 2025 (breakout ~Apr 22)',
      consolFrom: '2025-02-06', consolTo: '2025-04-22',
      preFrom: '2025-01-07', preTo: '2025-02-06',
    },
  ];

  // Also include TRON as false positive comparison
  cases.push({
    symbol: 'TRON', label: 'TRON — FALSE POSITIVE (concordant decline Jan-Feb 2026)',
  ...
  console.log('\n  ── Key Metrics ──');
  const hdr = '  ' + 'Ticker'.padEnd(8) + 'Price∆'.padStart(8) + 'Net∂%'.padStart(8) + 'Slope'.padStart(8) +
    'Corr'.padStart(7) + 'Shift'.padStart(8) + 'AccWk%'.padStart(8) + 'Best'.padStart(7) + 'BestWin'.padStart(22);
  console.log(hdr);
  ...
  console.log('\n  ── Volatility Contraction ──');
  const hdr2 = '  ' + 'Ticker'.padEnd(8) + 'RngT1'.padStart(8) + 'RngT3'.padStart(8) + 'Rng∆'.padStart(8) +
    'VolT1'.padStart(9) + 'VolT3'.padStart(9) + 'Vol∆'.padStart(8) + 'BB∆'.padStart(8);
  console.log(hdr2);
  ...
  const hdr3 = '  ' + 'Ticker'.padEnd(8) + 'L5 ∂%'.padStart(8) + 'L5 Rng'.padStart(8) + 'Avg Rng'.padStart(8) +
    'L5 Vol'.padStart(9) + 'Avg Vol'.padStart(9) + 'VolDry'.padStart(8) + 'RngComp'.padStart(8);
  console.log(hdr3);
  ...
  console.log('\n  ── Absorption Pattern ──');
  const hdr4 = '  ' + 'Ticker'.padEnd(8) + 'Absorb%'.padStart(9) + 'StrongAbs'.padStart(10) + 'Distrib%'.padStart(10) + 'PricePos'.padStart(9);
  console.log(hdr4);
  ...
  for (const r of allResults) {
    ...
    console.log(`  ${r.symbol.padEnd(8)} 3σ outliers: ${o3}   2σ outliers: ${o2}   vol outliers: ${r.volOutliers.length}`);
  }
