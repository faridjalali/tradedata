/**
 * VDF Deep Analysis — CRDO
 * Full period: 1/15/25 - 5/15/25
 * Likely accumulation: 2/1/25 - 4/22/25
 * Breakout: ~4/22/25
 */
...
function scoreSubwindow(dailySlice, preDaily, opts = {}) {
  ...
  // Weekly cum delta slope (using capped values)
  const weeklyDeltas = [];
  let dayIdx = 0;
...
(async () => {
  console.log('=== VDF Deep Analysis — CRDO ===\n');

  console.log('Fetching full period (1/15/25 → 5/15/25)...');
  const allBars = await fetch1mChunked('CRDO', '2025-01-15', '2025-05-15');
  console.log('Fetching pre-context (12/15/24 → 1/15/25)...');
  const preBars = await fetch1mChunked('CRDO', '2024-12-15', '2025-01-15');
  ...
  console.log('  CRDO — FULL PERIOD OVERVIEW (1/15/25 → 5/15/25)');
  console.log('='.repeat(80));
  ...
  console.log(`\n${'='.repeat(80)}`);
  console.log('  CRDO — USER-SPECIFIED ACCUMULATION WINDOW (2/1 → 4/22)');
  console.log('='.repeat(80));

  const accumDaily = allDaily.filter(d => d.date >= '2025-02-01' && d.date <= '2025-04-22');
  const directResult = scoreSubwindow(accumDaily, preDaily);
  if (directResult) {
    console.log(`  Period: ${accumDaily[0]?.date} → ${accumDaily[accumDaily.length-1]?.date} (${accumDaily.length} days)`);
    const closes = accumDaily.map(d => d.close);
    console.log(`  Price: $${closes[0]?.toFixed(2)} → $${closes[closes.length-1]?.toFixed(2)} (${((closes[closes.length-1]-closes[0])/closes[0]*100).toFixed(1)}%)`);
    console.log(`  Score: ${directResult.score.toFixed(4)} ${directResult.detected ? '✅ DETECTED' : '❌ NOT DETECTED'}`);
    if (directResult.reason) console.log(`  Reason: ${directResult.reason}`);
    if (directResult.deltaSlopeNorm !== undefined) {
      console.log(`  Slope: ${directResult.deltaSlopeNorm.toFixed(2)}  Net∂: ${directResult.netDeltaPct.toFixed(2)}%  Corr: ${directResult.priceDeltaCorr?.toFixed(2) || '?'}  AccWk: ${directResult.accumWeeks}/${directResult.weeks}`);
    }
    if (directResult.cappedDays?.length > 0) {
      console.log('  Capped days:');
      for (const c of directResult.cappedDays) console.log(`    ${c.date}: ${(c.original/1000).toFixed(0)}K → ${(c.capped/1000).toFixed(0)}K`);
    }
    if (directResult.components) {
      const c = directResult.components;
      console.log(`  Components: s1=${c.s1.toFixed(2)} s2=${c.s2.toFixed(2)} s3=${c.s3.toFixed(2)} s4=${c.s4.toFixed(2)} s5=${c.s5.toFixed(2)} s6=${c.s6.toFixed(2)} s7=${c.s7.toFixed(2)}`);
    }
    if (directResult.weeksData) {
      console.log('  Weekly breakdown:');
      for (const w of directResult.weeksData) {
        const dir = w.effectiveDelta >= 0 ? '+' : '';
        console.log(`    ${w.weekStart}: ${dir}${w.deltaPct.toFixed(2)}%  (${dir}${(w.delta/1000).toFixed(0)}K)  rng=${w.avgRange.toFixed(2)}%  vol=${(w.avgVol/1e6).toFixed(2)}M  [${w.nDays}d]`);
      }
    }
  } else {
    console.log('  insufficient data or gated');
  }
  ...
  if (noCap) {
    ...
    if (noCap.deltaSlopeNorm !== undefined) console.log(`    Slope: ${noCap.deltaSlopeNorm.toFixed(2)}  Net∂: ${noCap.netDeltaPct.toFixed(2)}%`);
  }

  // ---- Subwindow scanning ----
  console.log(`\n${'='.repeat(80)}`);
  console.log('  CRDO — SUBWINDOW SCANNING (across full 1/15 → 5/15 period)');
  console.log('='.repeat(80));
  ...
  if (allResults.length > 0) {
    ...
    console.log(`\n${'='.repeat(80)}`);
    console.log('  CRDO — BEST SUBWINDOW DETAIL');
    console.log('='.repeat(80));
    ...
    console.log(`  Price: ${best.overallPriceChange.toFixed(1)}%  Net∂: ${best.netDeltaPct.toFixed(2)}%  Corr: ${best.priceDeltaCorr.toFixed(2)}  Slope: ${best.deltaSlopeNorm.toFixed(2)}`);
    ...
    console.log(`  Absorption: ${best.absorptionPct.toFixed(1)}%  Buy vs Sell: ${best.largeBuyVsSell.toFixed(1)}  Vol decline: ${best.volDeclineScore.toFixed(2)}`);

    const c = best.components;
    console.log(`\n  Score Components (proposed weights):`);
    console.log(`    s1 Net Delta (25%):       ${c.s1.toFixed(4)}  → ${(c.s1 * 0.25).toFixed(4)}`);
    console.log(`    s2 Delta Slope (22%):     ${c.s2.toFixed(4)}  → ${(c.s2 * 0.22).toFixed(4)}`);
    console.log(`    s3 Delta Shift (15%):     ${c.s3.toFixed(4)}  → ${(c.s3 * 0.15).toFixed(4)}`);
    console.log(`    s4 Accum Wk Ratio (15%):  ${c.s4.toFixed(4)}  → ${(c.s4 * 0.15).toFixed(4)}`);
    console.log(`    s5 Buy vs Sell (10%):     ${c.s5.toFixed(4)}  → ${(c.s5 * 0.10).toFixed(4)}`);
    console.log(`    s6 Absorption (8%):       ${c.s6.toFixed(4)}  → ${(c.s6 * 0.08).toFixed(4)}`);
    console.log(`    s7 Vol Decline (5%):      ${c.s7.toFixed(4)}  → ${(c.s7 * 0.05).toFixed(4)}`);

    if (best.cappedDays?.length > 0) {
      console.log('\n  Outlier days capped:');
      for (const cd of best.cappedDays) console.log(`    ${cd.date}: ${(cd.original/1000).toFixed(0)}K → ${(cd.capped/1000).toFixed(0)}K`);
    }

    console.log('\n  Weekly breakdown:');
    for (const w of best.weeksData) {
      const dir = w.effectiveDelta >= 0 ? '+' : '';
      console.log(`    ${w.weekStart}: ${dir}${(w.effectiveDelta/1000).toFixed(0)}K  ∂%=${dir}${w.deltaPct.toFixed(2)}%  rng=${w.avgRange.toFixed(2)}%  vol=${(w.avgVol/1e6).toFixed(2)}M  [${w.nDays}d]`);
    }
    ...
    for (let i = 0; i < bestSlice.length; i++) {
      ...
      console.log(`    ${d.date}: $${d.close.toFixed(2).padStart(7)} (${priceChg.padStart(6)}%)  ∂=${dir}${(d.delta/1000).toFixed(0).padStart(6)}K  (${d.deltaPct.toFixed(1).padStart(6)}%)  rng=${d.rangePct.toFixed(2).padStart(6)}%  vol=${(d.totalVol/1e6).toFixed(2)}M${absorb}`);
    }
  } else {
    ...
    for (const winSize of windowSizes) {
      ...
      for (let start = 0; start <= allDaily.length - winSize; start++) {
        const slice = allDaily.slice(start, start + winSize);
        const result = scoreSubwindow(slice, preDaily);
        if (result) allNon.push({ start, winSize, ...result, startDate: slice[0].date, endDate: slice[slice.length - 1].date });
      }
    }
  ...
  console.log(`\n${'='.repeat(80)}`);
  console.log('  CRDO — PRE-BREAKOUT (10 days ending 4/22)');
  console.log('='.repeat(80));

  const preBreakout = allDaily.filter(d => d.date <= '2025-04-22').slice(-10);
  ...
  console.log(`  Last 5d avg range: ${last5Range.toFixed(2)}% (overall avg: ${avgRange.toFixed(2)}%, change: ${((last5Range/avgRange - 1)*100).toFixed(0)}%)`);
  console.log(`  Last 5d avg vol: ${(last5Vol/1e6).toFixed(2)}M (overall avg: ${(avgVol/1e6).toFixed(2)}M, change: ${((last5Vol/avgVol - 1)*100).toFixed(0)}%)`);
  ...
  console.log(`\n  POST-BREAKOUT (after 4/22):`);
  const postBreakout = allDaily.filter(d => d.date > '2025-04-22').slice(0, 15);
  for (const d of postBreakout) {
    const dir = d.delta >= 0 ? '+' : '';
    console.log(`    ${d.date}: $${d.close.toFixed(2).padStart(7)}  ∂=${dir}${(d.delta/1000).toFixed(0).padStart(6)}K  (${d.deltaPct.toFixed(1).padStart(6)}%)  vol=${(d.totalVol/1e6).toFixed(2)}M`);
  }
