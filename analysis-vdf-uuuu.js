/**
 * VDF Deep Analysis — UUUU (Energy Fuels)
 * Full period: 10/14/25 - 1/29/26
 * Breakout: ~1/2/26
 */
...
(async () => {
  console.log('=== VDF Deep Analysis — UUUU ===\n');

  console.log('Fetching full period (10/14/25 → 1/29/26)...');
  const allBars = await fetch1mChunked('UUUU', '2025-10-14', '2026-01-29');
  console.log('Fetching pre-context (9/14/25 → 10/14/25)...');
  const preBars = await fetch1mChunked('UUUU', '2025-09-14', '2025-10-14');
  ...
  console.log('  UUUU — FULL PERIOD OVERVIEW (10/14/25 → 1/29/26)');
  console.log('='.repeat(80));
  ...
  // Subwindow scanning
  console.log(`\n${'='.repeat(80)}`);
  console.log('  UUUU — SUBWINDOW SCANNING');
  console.log('='.repeat(80));
  ...
  if (zones.length > 0) {
    console.log(`\n${'='.repeat(80)}`);
    console.log(`  UUUU — ${zones.length} ACCUMULATION ZONE(S) DETECTED`);
    console.log('='.repeat(80));
    ...
    console.log('\n  Top 10 highest-scoring windows:');
    for (const w of allWindows.slice(0, 10)) {
  ...
  // Pre-breakout around 1/2/26
  console.log(`\n${'='.repeat(80)}`);
  console.log('  UUUU — PRE-BREAKOUT (10 days ending ~1/2/26)');
  console.log('='.repeat(80));

  const preBreakout = allDaily.filter(d => d.date <= '2026-01-02').slice(-10);
  ...
  console.log(`\n  POST-BREAKOUT (after 1/2/26):`);
  const postBreakout = allDaily.filter(d => d.date > '2026-01-02').slice(0, 15);
