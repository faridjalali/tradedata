/**
 * VDF Deep Analysis — MOD (Modine Manufacturing)
 * Full period: 1/29/25 - 9/18/25
 * True Breakout: 4/22/25
 */
...
(async () => {
  console.log('=== VDF Deep Analysis — MOD ===\n');

  console.log('Fetching full period (1/29/25 → 9/18/25)...');
  const allBars = await fetch1mChunked('MOD', '2025-01-29', '2025-09-18');
  console.log('Fetching pre-context (12/29/24 → 1/29/25)...');
  const preBars = await fetch1mChunked('MOD', '2024-12-29', '2025-01-29');
  ...
  console.log('='.repeat(80));
  console.log('  MOD — FULL PERIOD OVERVIEW (1/29/25 → 9/18/25)');
  console.log('='.repeat(80));
  ...
  // Subwindow scanning
  console.log(`\n${'='.repeat(80)}`);
  console.log('  MOD — SUBWINDOW SCANNING');
  console.log('='.repeat(80));
  ...
  if (zones.length > 0) {
    console.log(`\n${'='.repeat(80)}`);
    console.log(`  MOD — ${zones.length} ACCUMULATION ZONE(S) DETECTED`);
    console.log('='.repeat(80));
  ...
  // RED DELTA & VOL
  analyzeRedDeltaAndVolatility(allDaily, '2025-04-22');

  // BREAKOUT PROXIMITY
  analyzeBreakoutProximity(allDaily, '2025-04-22');

  // Breakout and post-breakout
  const breakoutIdx = allDaily.findIndex(d => d.date >= '2025-04-22');
  if (breakoutIdx > 0 && breakoutIdx < allDaily.length) {
    console.log(`\n${'='.repeat(80)}`);
    console.log('  MOD — BREAKOUT & POST-BREAKOUT');
    console.log('='.repeat(80));
