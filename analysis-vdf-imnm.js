...
function analyzeBreakoutProximity(allDaily, breakoutDate) {
  ...
  for (let i = 0; i < last30.length; i++) {
    if (last30[i].delta < 0) {
      ...
      if (streak >= 2) {
        ...
        console.log(
          `    ${streakStart}→${last30[i - 1].date}: ${streak} red days, deltas: ${streakDeltas.map((d) => (d / 1000).toFixed(0) + 'K').join(', ')}${label}`,
        );
      }
      streak = 0;
    }
  ...
  if (streak >= 2) {
    ...
    const label = increasing ? ' [INTENSIFYING]' : decreasing ? ' [FADING]' : '';
    console.log(
      `    ${streakStart}→${last30[last30.length - 1].date}: ${streak} red days, deltas: ${streakDeltas.map((d) => (d / 1000).toFixed(0) + 'K').join(', ')}${label}`,
    );
  }
  ...
  for (let i = Math.max(5, n - 30); i < n; i++) {
    ...
    const bar =
      delta5d > 0
        ? '+'.repeat(Math.min(25, Math.round(Math.abs(delta5d) / 1000 / 5)))
        : '-'.repeat(Math.min(25, Math.round(Math.abs(delta5d) / 1000 / 5)));
    console.log(
      `    [${String(daysLeft).padStart(2)}d] ${preBreakout[i].date}: 5d∂=${(delta5d / 1000).toFixed(0).padStart(6)}K  |${bar}|`,
    );
  }
