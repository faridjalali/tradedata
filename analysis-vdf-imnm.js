/**
 * VDF Deep Analysis — IMNM (Immunomedics / ImmunoGen)
 * Full period: 7/23/25 - 10/22/25
 * Breakout: 9/18/25
 */
...
function analyzeBreakoutProximity(allDaily, breakoutDate) {
  ...
  console.log(`  Breakout: ${breakoutDate}`);
  ...
  // Day-by-day last 20 days
  console.log(`\n  LAST 20 DAYS BEFORE BREAKOUT:`);
  ...
  for (let i = 0; i < last20.length; i++) {
    ...
    const daysLeft = last20.length - i;
  ...
  console.log(`\n  SELLER EXHAUSTION DETECTION (consecutive red delta streaks, last 30d):`);
  ...
  for (let i = 0; i < last30.length; i++) {
    if (last30[i].delta < 0) {
      if (streak === 0) { streakStart = last30[i].date; streakDeltas = []; }
      streak++;
      streakDeltas.push(last30[i].delta);
    } else {
      if (streak >= 2) {
        const increasing = streakDeltas.length >= 2 && Math.abs(streakDeltas[streakDeltas.length-1]) > Math.abs(streakDeltas[0]);
        const decreasing = streakDeltas.length >= 2 && Math.abs(streakDeltas[streakDeltas.length-1]) < Math.abs(streakDeltas[0]);
        const label = increasing ? ' [INTENSIFYING]' : decreasing ? ' [FADING]' : '';
        console.log(`    ${streakStart}→${last30[i-1].date}: ${streak} red days, deltas: ${streakDeltas.map(d => (d/1000).toFixed(0)+'K').join(', ')}${label}`);
      }
      streak = 0;
    }
  }
  if (streak >= 2) {
    ...
    const decreasing = streakDeltas.length >= 2 && Math.abs(streakDeltas[streakDeltas.length-1]) < Math.abs(streakDeltas[0]);
    const label = increasing ? ' [INTENSIFYING]' : decreasing ? ' [FADING]' : '';
    console.log(`    ${streakStart}→${last30[last30.length-1].date}: ${streak} red days, deltas: ${streakDeltas.map(d => (d/1000).toFixed(0)+'K').join(', ')}${label}`);
  }
  ...
  console.log(`\n  CUM DELTA ACCELERATION (5-day rolling, last 30d):`);
  let runCumDelta = 0;
  const cumDeltas = [];
  ...
  for (let i = Math.max(5, n - 30); i < n; i++) {
    ...
    const daysLeft = n - i;
    const bar = delta5d > 0 ? '+'.repeat(Math.min(25, Math.round(Math.abs(delta5d)/1000/5))) : '-'.repeat(Math.min(25, Math.round(Math.abs(delta5d)/1000/5)));
    console.log(`    [${String(daysLeft).padStart(2)}d] ${preBreakout[i].date}: 5d∂=${(delta5d/1000).toFixed(0).padStart(6)}K  |${bar}|`);
  }
...
(async () => {
  console.log('=== VDF Deep Analysis — IMNM ===\n');

  console.log('Fetching full period (7/23/25 → 10/22/25)...');
  const allBars = await fetch1mChunked('IMNM', '2025-07-23', '2025-10-22');
  console.log('Fetching pre-context (6/23/25 → 7/23/25)...');
  const preBars = await fetch1mChunked('IMNM', '2025-06-23', '2025-07-23');
  ...
  console.log('='.repeat(80));
  console.log('  IMNM — FULL PERIOD OVERVIEW (7/23/25 → 10/22/25)');
  console.log('='.repeat(80));
  ...
  // Subwindow scanning
  console.log(`\n${'='.repeat(80)}`);
  console.log('  IMNM — SUBWINDOW SCANNING');
  console.log('='.repeat(80));
  ...
  if (zones.length > 0) {
    console.log(`\n${'='.repeat(80)}`);
    console.log(`  IMNM — ${zones.length} ACCUMULATION ZONE(S) DETECTED`);
    console.log('='.repeat(80));
  ...
  // RED DELTA & DELTA VOL
  analyzeRedDeltaAndVolatility(allDaily, '2025-09-18');

  // BREAKOUT PROXIMITY
  analyzeBreakoutProximity(allDaily, '2025-09-18');

  // Breakout and post-breakout
  const breakoutIdx = allDaily.findIndex(d => d.date >= '2025-09-18');
  if (breakoutIdx > 0 && breakoutIdx < allDaily.length) {
    console.log(`\n${'='.repeat(80)}`);
    console.log('  IMNM — BREAKOUT & POST-BREAKOUT');
    console.log('='.repeat(80));
